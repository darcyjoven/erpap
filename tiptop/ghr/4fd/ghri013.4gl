...
DEFINE
    ...
    CALL cl_set_comp_visible("hrav06,hrat01",FALSE)
    CALL cl_set_comp_entry("hrav03,hrat02",TRUE)
    CALL cl_set_comp_entry("hrav02,hrav06,hrav07",FALSE)
    DISPLAY g_hrav01 TO hrav01
    CALL cl_opmsg('b')  
    LET g_forupd_sql = " SELECT DISTINCT '','','',hrav04,hrav05,'','','','','','',hrav07,'',hrav08,hrav09,hrav10 FROM hrav_file WHERE hrav01=? AND hrav04=? AND hrav05 = ? FOR UPDATE "
     ...
     INPUT ARRAY g_hrav WITHOUT DEFAULTS FROM s_hrav.*  
        ...
        BEFORE ROW
            ...
            IF g_rec_b_2 >= l_ac_2 THEN
                ...
                OPEN i013_1_b_cur2 USING g_hrav01,g_hrav_t.hrav04,g_hrav_t.hrav05
                IF STATUS THEN
        ...
        AFTER FIELD hrav04                       
            IF g_hrav[l_ac_2].hrav04 IS NOT NULL THEN 
               ...
               SELECT count(*) INTO l_n
                 FROM hrav_file
                WHERE hrav04 = g_hrav[l_ac_2].hrav04
               ...
               IF g_hrav[l_ac_2].hrav05 IS NOT NULL THEN
                  ...
                  SELECT count(*) INTO l_n
                    FROM hrav_file
                   WHERE hrav04 = g_hrav[l_ac_2].hrav04
                     AND hrav05 = g_hrav[l_ac_2].hrav05
                  IF l_n = 0 OR cl_null(l_n) THEN
                     CALL cl_err('',-217,0)
                     LET g_hrav[l_ac_2].hrav04 = g_hrav_t.hrav04
                     NEXT FIELD hrav04
        ...
        BEFORE DELETE                            
            IF g_hrav_t.hrav04 IS NOT NULL THEN
                ...
                LET l_sql = "DELETE FROM hrav_file ",
                            ...
                            "   AND hrav01 = '",g_hrav01,"'",
                            "   AND hrav03 IN (SELECT hratid FROM hrat_file WHERE ",g_wc2," )" 
                IF SQLCA.sqlcode THEN 
...
FUNCTION i013_getName()
DEFINE l_hrat      RECORD LIKE hrat_file.* 
IF g_hrav[l_ac_2].hrav05 = 'hrat04' THEN
		LET l_hrat.hrat04 = g_hrav[l_ac_2].hrav06
		CALL i006_hrat04(l_hrat.*) RETURNING g_hrav[l_ac_2].hrav06_n
END IF

IF g_hrav[l_ac_2].hrav05 = 'hrat17' THEN
		CALL s_code('333',g_hrav[l_ac_2].hrav06) RETURNING g_hrag.*
		LET g_hrav[l_ac_2].hrav06_n = g_hrag.hrag07
END IF

IF g_hrav[l_ac_2].hrav05 = 'hrat21' THEN
		CALL s_code('337',g_hrav[l_ac_2].hrav06) RETURNING g_hrag.*
		LET g_hrav[l_ac_2].hrav06_n = g_hrag.hrag07
END IF
