...
#                  "      SET o.wsaleamt = o.wsaleamt+ x.omb16 ",      #No.TQC-D90012
      ...
      IF tm.c='Y' THEN 
         ...
         EXECUTE q761_ckl         
         #darcy:2023/03/24 add s---
         delete from axcq761_ins
         let l_sql = "insert into axcq761_ins SELECT sysdate,tlf01,
                      CASE wsaleamt WHEN 0 THEN 0 ELSE tlfc21 / wsaleamt END,'",g_user,"'
                      FROM (SELECT tlf01, NVL(SUM(tlfc21), 0) tlfc21, NVL(SUM(wsaleamt), 0) wsaleamt
                              FROM AXCQ761_TMP x GROUP BY tlf01)"
         prepare q761_ins_tmp from l_sql
         execute q761_ins_tmp
         #darcy:2023/03/24 add e---
      END IF 
#FUN-D10022----------------add-------------end-----------------------        
   END FOR    
