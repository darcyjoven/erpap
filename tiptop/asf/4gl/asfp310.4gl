...
define flag_atmt260  like type_file.num5   #darcy:2023/05/15 add
...
MAIN
   ...
   IF (NOT cl_setup("ASF")) THEN
      EXIT PROGRAM
   END IF
   let flag_atmt260 = false #darcy:2023/05/15 add
...
################################################################################
   ...
   END IF
   #FUN-A80102(E)
   call p310_set_imaud06()#darcy:2023/06/15 add
...
#                 CALL cl_err(g_sfb01,'asf-312',0)   #No.FUN-660128
               ...
               SELECT sum(shm08) INTO l_sum FROM shm_file WHERE shm012 = g_sfb01
                ...
                #darcy:2023/06/06 add s---
                let l_ima56 = p310_get_ima56(g_sfb01)
                #darcy:2023/06/06 add e---
                IF g_p = "5" THEN
                    LET l_ima56 = ' '
                END IF 
                #NO.xyb-end
...
#              LET l_t1=g_sfb01[1,3]
               ...
               #darcy:2023/05/15 add s---
               let l_cnt = 0
               select count(1) into l_cnt from tsc_file where tscud03 = g_sfb01
               display 'N' to formonly.isatmt260
               if l_cnt > 0 then
                  #是下线工单，根据旧工单计算
                  let l_pcs = 0
                  let l_pnl = 0
                  select sum(tsc05),sum(case imaud10 when 0 then 0 else tsc05/imaud10 end)
                    into l_pcs,l_pnl from tsc_file,ima_file,sfb_file
                   where tscud03 = g_sfb01 and sfb05 = ima01 and sfb01 = tsc14[1,12]
                     and tscpost='Y' and tscconf ='Y' 
                  if l_pcs != g_sfb08 or g_ynum1 !=0 then
                     call cl_err(l_pcs,'ctm-004',1)
                  else
                     let g_lot = 0
                     let flag_atmt260 = true
                     display 'Y' to formonly.isatmt260
                  end if
                  display g_ynum1  to formonly.ynum1
                  display g_sfb08  to formonly.sfb08
               end if
               #darcy:2023/05/15 add e---
         ...
         AFTER FIELD g_lot        #批量 
            #darcy:2023/05/15 add s---
            if g_lot = 0 and l_cnt <=0 then
               next field g_lot
            end if
            #darcy:2023/05/15 add e---
            IF cl_null(g_lot) OR g_lot < 0 OR g_lot > g_sfb08 THEN #darcy:2023/05/15 modify
               NEXT FIELD g_lot
            END IF
...
#           #          CALL FGL_DIALOG_SETBUFFER( g_slip )
      ...
      ELSE
        #参考原下线批单数量拆批
        let l_i = 1
        declare p310_atmt260 cursor for 
            select rownum,tsc05 from tsc_file,shb_file where tscud03 = g_sfb01
              and shb01 = tscud02 and tscpost = 'Y' and tscconf = 'Y'  order by shb16,tsc01 #darcy:2024/07/12 add tsc01
        
        foreach p310_atmt260 into new[l_i].seq,new[l_i].woqty
            if sqlca.sqlcode then
                call cl_err('p310_atmt260',sqlca.sqlcode,1)
                exit foreach
            end if
            let l_i = l_i + 1
        end foreach
        call new.deleteElement(l_i)
        let l_i = l_i - 1
        let l_seq = l_i
      #darcy:2023/05/15 add e---
      END IF
      ...
      DECLARE max_pixuhao CURSOR FROM " SELECT pixuhao  FROM shm05_file WHERE pixuhaoyear = ?  and pixuhaomonth = ? and pixuhaoday= ? FOR UPDATE nowait "
      BEGIN WORK
         OPEN max_pixuhao USING l_year,l_month,l_day
         FETCH max_pixuhao INTO l_max_pixuhao
         call cl_log_string("批序号："||l_max_pixuhao)

         IF cl_null(l_max_pixuhao) OR l_max_pixuhao = 0 THEN
            LET l_max_pixuhao = 1
            INSERT INTO shm05_file VALUES(
               l_max_pixuhao,l_year,l_month,l_day
            )
            call cl_log_string("更新后批次号："||l_max_pixuhao)
         ELSE
      ...
      #add by lixwz201225 e---
      call cl_log_string("本次取号："||l_ta_shm05)
      call cl_log_string("---"||l_now||" user:"||g_user||"结束取号---")
...
#  DEFINE l_qty           LIKE ima_file.ima26,         #No.FUN-680121 DEC(15,3)
   ...
   #add by lixwz201225 e---
   define l_cnt integer    #darcy:2024/01/31 add
       ...
       #    LET g_success = 'N' RETURN  END IF  #mark by guanyao160615
        ...
        if i=1 then
         ...
         select count(1) into l_cnt from shm_file
            where ta_shm05 = g_shm.ta_shm05
         if l_cnt > 0 then
               call s_errmsg('ta_shm05',g_shm.ta_shm05,g_shm.shm01,'csf-120',1)
               let g_success='N'
               continue for
         end if
        end if
        ...
        call asfp310_upd_shm05(g_shm.ta_shm05,1) #darcy:2024/03/11 add
...
DEFINE l_round     LIKE type_file.num15_3
   ...
   FOREACH sfb_cur INTO l_sfb[l_ac].*
      ...
      #darcy:2023/06/19 mark s---
      #  IF l_sfb[l_ac].sfb08_1>l_sfb[l_ac].imaud10*100 THEN
      #          LET  l_sfb[l_ac].sfb08_1=l_sfb[l_ac].imaud10*100
      # END IF
      #darcy:2023/06/19 mark e---
      #darcy:2023/06/19 add s---
      # if l_sfb[l_ac].sfb08_1 > g_lot then
      #    let l_sfb[l_ac].sfb08_1 = g_lot
      # end if
      #darcy:2023/06/19 add e---
      #darcy:2023/06/27 add s---
      # PNL=g_lot/g_imaud10
      if l_sfb[l_ac].sfb08_1 > l_sfb[l_ac].imaud10*g_lot/g_imaud10 then
         let l_sfb[l_ac].sfb08_1 = l_sfb[l_ac].imaud10*g_lot/g_imaud10
      end if
    ...
    FOREACH tmp_cur INTO l_tmp.*
      ...
      IF l_tmp.num2 >0 THEN      #add by jixf 160809
       #IF g_lot >0 THEN
         ...
         END IF 
         #darcy:2023/05/15 add s---
         if flag_atmt260 then
            #如果是下线
            let l_seq = new.getlength()
         end if
         #darcy:2023/05/15 add e---
         FOR l_i=1 TO l_seq
            IF l_i = l_seq THEN 
            ...
            #    let l_cnt = 0 
            #    select count(1) into l_cnt from shm_file
            #       where ta_shm05 = l_shm.ta_shm05
            #    if l_cnt > 0 then
            #          call s_errmsg('ta_shm05',l_shm.ta_shm05,l_shm.shm01,'csf-120',1)
            #          let g_success='N'
            #          continue for
            #    end if
            #  end if
             #darcy:2024/01/31 add e---
             call asfp310_upd_shm05(l_shm.ta_shm05,1) #darcy:2024/03/11 add
...
function p310_get_ima56(p_sfb01)
   ...
   define l_tc_sma19 varchar(40)
   define l_tc_sma04 varchar(40)
   define l_tc_sma03  integer
   define l_lot  like ima_file.ima56

   # 默认100PNL
   SELECT sfbud10*100 INTO l_ima56 FROM sfb_file WHERE sfb01 = p_sfb01
   
   # 检查是否是光板
   let l_tc_sma04 = 0
   let l_sfb05 = null
   let l_sfbud10 = null
   select sfb05,sfbud10 into l_sfb05,l_sfbud10 from sfb_file,ima_file 
    where sfb05 = ima01  and sfb01 = p_sfb01
   # 检查是否在csmi101配置中
 

   SELECT imaud06,
          CASE WHEN imaud10 >=tc_sma07 THEN tc_sma09*sfbud10 ELSE '100'*sfbud10 END tc_sma092
     INTO l_tc_sma03,l_lot
   FROM sfb_file,ima_file
   LEFT JOIN tc_sma_file ON tc_sma01='csmi102' AND tc_sma02 = 'aimi100' AND tc_sma03 =imaud06
   WHERE sfb01 = p_sfb01 AND sfb05=ima01 

   # if cl_null(l_lot) or l_lot<=0 then
   #    display 0 to tc_sma04
   #    return l_ima56
   # end if 

   display l_tc_sma03 to tc_sma04

   return cs_split(l_sfb05)*l_sfbud10 #darcy:2024/01/04 add
   
   return l_lot
end function
#darcy:2023/06/06 add e---
#darcy:2023/06/15 add s---
function p310_set_imaud06()
   define l_field,l_value string
   define l_tc_sma03       varchar(20)
   define l_tc_sma06       varchar(200)

   declare p310_imaud06_cl cursor for
      select tc_sma03,tc_sma06 from tc_sma_file 
       where tc_sma01='csmi102' and tc_sma02='aimi100'
         and tc_sma20 ='Y'
       order by tc_sma03
   
   foreach p310_imaud06_cl into l_tc_sma03,l_tc_sma06
      if sqlca.sqlcode then
         call cl_err('p310_imaud06_cl',sqlca.sqlcode,1)
         exit foreach
      end if
...
# 重新更新一次shm05_file
function asfp310_upd_shm05(p_ta_shm05,p_shm05)
