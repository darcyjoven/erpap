...
DEFINE
    ...
    g_ecb06        like ecb_file.ecb06,  #darcy:2023/05/29 add
    ...
    g_sfa     DYNAMIC ARRAY OF RECORD    #程式變數(Prinram Variables)
                 ...
                 ecm03     like ecm_file.ecm03,   #darcy:2023/12/21 add
              ...
              IF NOT cl_null(g_sfb.sfb01) THEN
               ...
               #Mark FUN-B50026--end
                ...
                END IF
                 ...
                 #   如果单别是MSA则将申请人变为NULL
                 if g_sfb.sfb01 = 'MSA' then
                  let g_sfb.sfb44 = null
...
#FUN-C30114---end      
   ...
   #darcy:2023/05/29 add s---
    # 如果单别是MKT/MSG，继续录入重工开始的单别
    if g_sfb.sfb01[1,3] = "MKT" or g_sfb.sfb01[1,3] = "MSG" then
      call cl_set_comp_visible("ecb06",true)
      input g_ecb06 from ecb06
         before input
         on action controlp
            case
               when infield(ecb06)
                  CALL cl_init_qry_var()
                  LET g_qryparam.form     = "q_ecb06" #add by guanyao160505
                  let g_qryparam.arg1 = g_sfb.sfb05
                  let g_qryparam.arg2 = g_sfb.sfb06
                  CALL cl_create_qry() RETURNING g_ecb06
                  DISPLAY g_ecb06 to ecb06
                  NEXT FIELD ecb06
            end case
         after field ecb06
            if not cl_null(g_ecb06) then
               let l_cnt = 0
               select count(1) into l_cnt from ecb_file where ecb01=g_sfb.sfb05 and ecb02=g_sfb.sfb06
                and ecb06 = g_ecb06
               if l_cnt = 0 then
                  call cl_err(g_ecb06,'abm-750',1)
                  next field ecb06
               end if
            end if
         ON ACTION CONTROLG 
      ...
      call cl_set_comp_visible("ecb06",false)
    end if
    #darcy:2023/05/29 add e---
...
#FUN-AA0059 ---------------------end-------------------------------
               ...
               select ecm03 into g_sfa[l_ac].ecm03 from ecm_file
                where ecm01 = g_sfb.sfb01 and ecm04 = g_sfa[l_ac].sfa08
               display by name g_sfa[l_ac].ecm03
...
DEFINE l_flag         LIKE type_file.num5   #MOD-AC0336
      ...
      LET g_sql =   #str---add by huanglf160808
          "SELECT sfa012,'',sfa013,sfa27,sfa08,ecm03,sfa26,sfa28,sfa03,ima02,ima021,ima08,",   #FUN-A50066 #darcy:2023/12/21 add ecm03 
          ...
          " left join ecm_file on ecm01 = sfa01 and ecm04 = sfa08 ", #darcy:2023/12/21 add
...
FUNCTION i301_crrut(p_cmd)   #MOD-A40026 add p_cmd
  ...
  define l_ecm03   like ecm_file.ecm03    #darcy:2023/05/29 add
 ...
 #IF NOT cl_null(g_sfb.sfb06) THEN                         #MOD-A40026 mark
  IF NOT cl_null(g_sfb.sfb06) AND                          #MOD-A40026
    ...
    #MOD-AC0336--end--modify----------
      ...
      END IF
      #darcy:2023/05/29 add s---
      # 如果是MSG/MKT，删除ecb06 之上的工艺编号
      if (g_sfb.sfb01[1,3] = 'MKT' or g_sfb.sfb01[1,3] = 'MSG') and not cl_null(g_ecb06) then
         let l_ecm03 = 0
         select ecm03 into l_ecm03 from ecm_file where ecm01 = g_sfb.sfb01
          and ecm04 = g_ecb06
         if l_ecm03 > 0 then
            delete from ecm_file where ecm01 = g_sfb.sfb01
             and ecm03 < l_ecm03
...
#FUN-A50066--end--mark-------------------
   ...
   #MSG/MKT
   if (g_sfb.sfb01[1,3] = 'MKT' or g_sfb.sfb01[1,3] = 'MSG') and not cl_null(g_ecb06) then
      update sfa_file set sfa08 = g_ecb06 where sfa01 = g_sfb.sfb01 and sfa03 = g_sfb.sfb05
   end if
   #darcy:2023/05/29 add e---
...
function asfi301_hold (arg)
   ...
   #run "echo '\n\n\n---' >> /u1/out/darcy.txt "
   #run "echo '"||g_user||"' >> /u1/out/darcy.txt "
   
   if cl_null(g_sfb.sfb01) then
      return
   end if
   # #run "echo 'line "||__LINE__||"' >> /u1/out/darcy.txt "
