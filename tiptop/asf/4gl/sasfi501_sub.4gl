...
globals "../../config/darcy.global"    #darcy:2024/02/18 add
#FUN-CC0095---begin
GLOBALS
...
#2021113001 add----end----
   ...
   IF l_count_1 > 0 OR l_count_2 > 0 THEN 
     ...
     END IF 
   ELSE 
    if p_sfp01[1,3] != '105' and p_sfp01[1,3] != 'MRE' then #darcy:2024/05/11 add
    LET l_sfp.sfp03 = g_today   #add by wangxt170210
    end if #darcy:2024/05/11 add
   END IF 
   if p_sfp01[1,3] != '105' and p_sfp01[1,3] != 'MRE' then #darcy:2024/05/11 add
   LET l_sfp.sfp03 = g_today   #add by wangxt170210
   end if #darcy:2024/05/11 add
...
#TQC-C40174 -----------------End-------------------
    ...
    #IF l_sfp.sfp06 = '1' THEN  #FUN-740187
     ...
     # 结案amri506
     if not amri506sub_close(g_prog,l_sfp.sfp01,true,true) then
...
#   DEFINE l_sql  STRING #MOD-C60234   #MOD-D10094 mark
    ...
    FOREACH i501_s2_c INTO l_sfs.*, l_sfb.*
        ...
        IF l_sfs.sfs07 <> l_sfs.sfsud02 THEN 
         #   LET l_sfs.sfs08 = ' ' #darcy:2024/03/31 mark
        END IF 
...
#  DEFINE l_imaicd13   LIKE imaicd_file.imaicd13  #FUN-BA0050
   ...
   #darcy:2023/04/03 add s---
   define l_fifo_img dynamic array of fifo_img
   define l_cnt2     like type_file.num5
   #darcy:2023/04/03 add e---
...
#MOD-C10100 --end--
   ...
   FOREACH i501_chk_ima140 INTO l_sfq02
      IF STATUS THEN
         LET l_sfq02=NULL
      END IF
      EXIT FOREACH
...
#        END IF #MOD-DB0006
     ...
     #------------No:CHI-B50041  add
       ...
       #IF l_sfp.sfp06 MATCHES '[134]' AND         #MOD-A80053 mark
        ...
        END IF
        #darcy:2023/04/03 add s---
        if l_sfs.sfsud02 !='S005' and  l_sfs.sfsud02 !='YP002' then
         let l_fifo_img[l_cnt2].img01 = l_sfs.sfs04
         let l_fifo_img[l_cnt2].img02 = l_sfs.sfsud02
         let l_fifo_img[l_cnt2].img03 = l_sfs.sfs08
         let l_fifo_img[l_cnt2].img04 = l_sfs.sfs09
         let l_cnt2 = l_cnt2 + 1
        end if
        #darcy:2023/04/03 add e---
     END FOREACH
     #darcy:2023/04/03 add s---
     call l_fifo_img.deleteElement(l_cnt2)
     if g_prog = 'asfi511' then
      call cs_fifo(l_fifo_img)
      #darcy:2024/02/18 add s---
      if not cs_fifo_validity(l_fifo_img) then
         let g_success='N'
         return
      end if  
...
#MOD-C80077 add end-------------------------------
  ...
  #FUN-A20048 --begin
      ...
      #             sie13 = g_sfs_sif.sfs01,sie14 = g_sfs_sif.sfs02 
      #       WHERE sie01= g_sfs_sif.sfs04 AND sie02= g_sfs_sif.sfs07
      #      AND sie03=g_sfs_sif.hg_sfs_sif.sfs10
      #      AND sie07=g_sfs_sif.sfs06 AND sie08=g_sfs_sif.sfs27  #TQC-AC0298
      #      AND sie012=g_sfs_sif.sfs012   #FUN-A60028 
      #      AND sie013=g_sfs_sif.sfs013   #FUN-A60028 
      #      IF SQLCA.SQLERRD[3]=0 THEN
...
FUNCTION i501sub_x(p_sfp01,p_action_choice,p_call_transaction,p_type)     #CHI-D20010 
   ...
   #darcy:2023/06/15 add s---
   if not amri506sub_delete(g_prog,l_sfp.sfp01,true,true) then
      if p_call_transaction then
         rollback work
      end if
      return
...
DEFINE p_sfp03    LIKE sfp_file.sfp03      #add by guanyao170110
   ...
   LET l_seq= 1
   LET l_sql = "SELECT sfs01,sfs02,sfs04,sfs05,sfs06,sfs07,sfs08,sfs09,sfs10,sfsud02,sfsud07 FROM sfs_file WHERE sfs01 = '",l_sfp.sfp01,"'"
   # darcy:2024/03/31 增加sfs01，sfs02
   ...
   FOREACH i501_sfs04_curs1_1 INTO l_sfs.*
      ...
      IF l_sfs.sfsud07 > 0 THEN 
         ...
         #darcy:2024/03/31 add s---
         # 将调拨后储位更新到asfi511上
         update sfs_file set sfs08 = l_imn.imn16
          where sfs01 = l_sfs.sfs01 and sfs02 = l_sfs.sfs02
         #darcy:2024/03/31 add e---
