...
#           l_t1               VARCHAR(03),
            ...
            l_mod,l_seq,l_i    LIKE type_file.num5         #No.FUN-680121 SMALLINT
   ...
   DEFINE l_imaud10       LIKE ima_file.imaud10        #add by guanyao160713
...
################################################################################
   ...
   WHILE TRUE
      ...
      INPUT g_sfb01,g_lot,g_num2 WITHOUT DEFAULTS FROM g_sfb01,g_lot,num2    #add by jixf 160809
      ON ACTION locale
     ...
     ON ACTION help                    #No.TQC-770004
         ...
         AFTER FIELD g_sfb01      #工單 
            IF not cl_null(g_sfb01)  THEN 
               SELECT sfb05,sfb06,sfb08,sfb13,sfb15,sfb14,sfb16,sfb919,sfbud07 
                 INTO g_sfb05,g_sfb06,g_sfb08,g_sfb13,g_sfb15,g_sfb14,g_sfb16,g_sfb919   #FUN-A80102
                      ,g_sfbud07  #add by guanyao160715
                 FROM sfb_file
...
#              LET l_t1=g_sfb01[1,3]
               ...
               SELECT ima02,ima021,ima55,ima56,imaud10 INTO l_ima02,l_ima021,l_ima55,l_ima56,l_imaud10  
               ...
               IF SQLCA.sqlcode THEN 
                  ...
                  LET l_ima55 = ' ' 
                  LET l_ima56 = ' '
                  LET l_imaud10 = ''  #add by guanyao160730
               END IF
               ...
               DISPLAY l_imaud10 TO FORMONLY.imaud10  #add by guanyao160730
            END IF
...
#           #          CALL FGL_DIALOG_SETBUFFER( g_slip )
      ...
      IF g_lot >0 THEN
         ...
         LET l_mod = g_sfb08 MOD g_lot
         LET l_seq = (g_sfb08 /g_lot) 
         IF l_mod != 0 THEN
            LET l_seq = l_seq + 1 
         END IF
         FOR l_i=1 TO l_seq
             ...
             IF g_sfb08 < l_totqty THEN 
                LET l_woqty = g_sfb08 - l_totqty + l_woqty 
             END IF
             LET new[l_i].seq = l_i
...
#       IF g_i THEN 
      #No.FUN-550067 ---end---    
       #    LET g_success = 'N' RETURN  END IF  #mark by guanyao160615
        ...
        SELECT max(substr(shm01,l_x+2,3)) INTO l_temp FROM shm_file WHERE substr(shm01,l,l_x) = g_sfb01
...
DEFINE l_i         LIKE type_file.num5
   ...
   END IF     
   LET l_ac = 1
   LET l_sql = "SELECT 'N',sfb01,sfb05,sfb06,sfb08,sfb08,imaud10,imaud07,sfb13,sfb15,sfb14,sfb16,sfb919",  #str----add by huanglf160801
...
DEFINE l_sfbud07       LIKE sfb_file.sfbud07  #add by guanyao160715   
    ...
    FOREACH tmp_cur INTO l_tmp.*
      ...
      #END IF 
       ...
       #   LET l_tmp.imaud10 = 1
       #END IF 
      IF l_tmp.sfb08 >0 THEN 
       #IF g_lot >0 THEN
         LET l_totqty = 0  LET l_woqty = 0
         SELECT ceil(l_tmp.sfb08/l_tmp.sfb08_1) INTO l_seq FROM dual 
         IF l_seq = 0 THEN 
            LET l_seq = 1
         END IF 
         FOR l_i=1 TO l_seq
            IF l_i = l_seq THEN 
               LET l_totqty = l_tmp.sfb08-(l_seq-1)*l_tmp.sfb08_1
            ELSE 
