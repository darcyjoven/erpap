...
#FUN-C20101-----ADD-----END-------               
               ...
               IF g_success = "Y" THEN
                  CALL t370sub_y_upd(g_ina.ina01,'Y',FALSE) #FUN-B50138
                  call t370_b_fill(" 1=1") #darcy:2023/08/02
                  call t370_ef()  #darcy:2023/07/11 add
               END IF
               CALL t370_refresh() RETURNING g_ina.*  #FUN-B50138
         ...
         when "yanfa_upd"
            if cl_chk_act_auth() then
               if g_prog = 'aimt301' or g_prog = 'aimt311'then
                  call saimt370_upd_inbud02(g_ina.ina01,false)
       ...
       #@WHEN "過帳還原"
                          ...
                          #FUN-C30257 add END
                            ...
                            #TQC-D80002 --------End----------
                              IF cl_confirm('asf-663') THEN  #TQC-AC0216
                                 ...
                                 # darcy:2024/04/25 add s---
                                 # 还原过账的时候,同步删除MES记录
                                 call DelMesUseNoR(g_ina.ina01) returning l_ok,l_res
                                 if not l_ok then
                                    message l_res
                                 end if
                                 # darcy:2024/04/25 add e---
         ...
         when "amri506"
            if cl_chk_act_auth() then
               begin work
               call t370sub_amri506(g_ina.ina01)
               if g_success = 'Y' then
                  commit work
               else
                  rollback work
         ...
         when "unamri506"
            if cl_chk_act_auth() then
               begin work
               call t370sub_unamri506(g_ina.ina01)
               if g_success = 'Y' then
                  commit work
               else
                  rollback work
...
#FUN-AA0059 ---------------------start----------------------------
            IF NOT cl_null(g_inb[l_ac].inb04) THEN
              #IF NOT s_chk_item_no(g_inb[l_ac].att00,'') THEN     #TQC-C20029 mark
               ...
               END IF
               #darcy:2024/06/04 add s---
               # 检查有RD单号的料号，部门和理由码是否都是研发
               select imaud30 into l_imaud30 from ima_file where ima01 = g_inb[l_ac].inb04
               if not cl_null(l_imaud30) then
                  select ina04 into g_ina.ina04 from ina_file
                   where ina01 = g_ina.ina01
                  if g_ina.ina04 <> 'B16' then
                     call cl_err('','cim-032',1)
                     next field inb04
                  end if
               end if
...
#FUN-BC0062 --end--
       ...
       AFTER FIELD inb901
        ...
        AFTER FIELD inbud13
            ...
            if not cl_null(g_inb[l_ac].inbud13) and g_inb[l_ac].inbud13 != 0 then
                  if g_inb[l_ac].inbud13 < g_ina.ina03 then
                     call cl_err(g_inb[l_ac].inbud13,'cim-030',1)
                     next field inbud13
                  end if
            end if
...
#      DISPLAY ARRAY g_rvbs TO s_rvbs.* ATTRIBUTE(COUNT=g_rec_b1) #FUN-C20101----MODIFY------
	...
	ACCEPT DIALOG                   #No.FUN-530067 HCN TEST
      ...
      #darcy:2023/03/30 add s---
      on action amri506
         let g_action_choice = "amri506"
         exit dialog

      on action unamri506
         let g_action_choice = "unamri506"
         exit dialog
      #darcy:2023/03/30 add e---
...
FUNCTION t370_init()
    ...
    if g_prog ='aimt302' then
      ...
      call cl_set_comp_att_text("inbud13","指定有效期")
    end if
    #darcy:2024/01/05 add e---
...
DEFINE l_inaplant_desc   LIKE azp_file.azp02    #FUN-9C0090 ADD
   ...
   LET g_ina.inaud04='N'     #darcy:2023/07/06
   ...
   LET g_ina.inaud05 = '0'           #開立  #darcy:2023/07/06 add
   LET g_ina.inamksg = 'N'         #簽核否#FUN-550047
   LET g_ina.inaud03 = 'N'         #簽核否#darcy:2023/07/06 add
...
FUNCTION t370_a_inschk()
   ...
   IF cl_null(g_ina.inaud03) THEN
      LET  g_ina.inaud03 = g_smy.smyapr
   END IF
   IF cl_null(g_ina.inaud03) THEN
      LET  g_ina.inaud03 = 'N'
   END IF
...
FUNCTION t370_chk_ina01()
   ...
   IF NOT cl_null(g_ina.ina01) THEN
      ...
      #"CRY"单别设置默认申请人和部门编号
      if g_ina.ina01[1,3] = "CRY" then
         let g_ina.ina11 = '36682'
         let g_ina.ina04 = 'B16'
         display by name g_ina.ina11,g_ina.ina04
         call t370_ina11('a') #darcy:2023/09/08 add
      end if
      #darcy:2023/08/29 add e---
      # LET  g_ina.inamksg = g_smy.smyapr #darcy:2023/07/06 mark
      LET  g_ina.inaud03 = g_smy.smyapr #darcy:2023/07/06 add
      DISPLAY BY NAME g_ina.inamksg,g_ina.inaud03   #簽核否 #darcy:2023/07/06 add
   ELSE
      IF g_sma.sma120 = 'Y' AND g_sma.sma907 = 'Y' THEN
         LET lg_smy62 = ''
         CALL t370_refresh_detail()
...
# 回傳值  : 0 開單失敗; 1 開單成功
 ...
 THEN
       LET g_success = 'Y'
       select ina08 into  g_ina.ina08 from ina_file where ina01 = g_ina.ina01 #darcy:2023/07/11 add
      #  LET g_ina.ina08 = 'S'   #開單成功, 更新狀態碼為 'S. 送簽中' #darcy:2023/07/11 mark
       DISPLAY BY NAME g_ina.ina08
   ELSE
       LET g_success = 'N'
   END IF
...
FUNCTION t370_copy()
   ...
   INPUT l_newno,l_ina03,l_ina02 FROM ina01,ina03,ina02
      ...
      AFTER FIELD ina01
          ...
          #darcy:2023/08/29 add s---
          #"CRY"单别设置默认申请人和部门编号
          if l_newno[1,3] = "CRY" then
            display "36682","B16" to ina11,ina04
            call t370_ina11('a') #darcy:2023/09/08 add
          end if
          #darcy:2023/08/29 add e---
          IF (NOT li_result) THEN                                                                                                  
...
function saimt370_upd_inbud02(p_ina01,p_inTran)
   ...
   select * into g_ina.* from ina_file where ina01 = p_ina01

   if g_ina.inaconf = 'X' then
      call cl_err(p_ina01,'cim-092',0)
      return
   end if

   # darcy:2024/06/27 add s---
   # 直接更新imaud30
   let l_sql = "
   ...
   execute saimt370_upd_inbud02
   # darcy:2024/06/27 add e---


   select count(*) into l_cnt from inb_file where inb15='3006'
    and inb01 = p_ina01
   ...
   foreach saimt370_upd_inbud02_cl1 into sr.*
      if sqlca.sqlcode then
         call cl_err("saimt370_upd_inbud02_cl1",sqlca.sqlcode,1)
         exit foreach
      end if
      if g_ina.ina07 matches sr.tc_sma02||"*" then
         if cl_null(match.tc_sma02) then
            let match.* = sr.*
         else
            if LENGTH(sr.tc_sma02) > LENGTH(match.tc_sma02) then
               let match.* = sr.*
            end if
         end if
      end if
   ...
   if not cl_null(match.tc_sma02) then
      update inb_file set inbud02 = match.tc_sma06
       where inb01 = p_ina01 and inb15 ='3006'
      if sqlca.sqlcode then
