...
#                               ima901,ima902,ima881,ima73,             #No.FUN-8C0131
                                ...
                                ,imaud31,imaud42,imaud43  #darcy:2022/11/25 add'
   ...
   INPUT BY NAME g_ima.imaoriu,g_ima.imaorig,
      ...
      # g_ima.ima109,g_ima.ima903, g_ima.ima905,g_ima.ima24,g_ima.ima911,   #FUN-610080 加ima911    #TQC-AC0185 mark 
       ...
       #g_ima.ima1015, g_ima.ima1014,g_ima.ima1016,   #No.FUN-640010   #CHI-CA0073 mark 
        ...
        ,g_ima.imaud31
        ,g_ima.imaud42
        ,g_ima.imaud43  #darcy:2022/11/25 add
        ...
        BEFORE INPUT
            ...
            #darcy:2022/12/05 add s---
            if cl_null(g_ima.imaud25) or g_ima.imaud25 ='N' then
               call cl_set_comp_entry("imaud26",false)
            else
...
#No.FUN-D40110 ---------- add ----------- end ----------------------
       ...
       #DEV-D40019 add end------
        ...
        ON CHANGE ima02
           ...
           #darcy:2024/03/15 add s---
           if g_ima.ima01 matches 'E.*' then
               select count(*) into l_cnt from ima_file where ima02 = g_ima.ima02 and ima1010 = '1'
               if l_cnt > 0 then
	...
	IF cl_null(g_ima.ima36) THEN LET g_ima.ima36 = ' ' END IF  #FUN-D40103 add
        ...
        AFTER FIELD ima71
           IF g_ima.ima71 IS NOT NULL THEN
              ...
              END IF
           END IF
        #darcy:2022/11/25 add s---
         AFTER FIELD imaud31
           IF g_ima.imaud31 IS NOT NULL THEN
              IF g_ima.imaud31 <0 THEN
                 CALL cl_err('','mfg4012',0)
                 NEXT FIELD imaud31
              END IF
           END IF
        #darcy:2022/11/25 add e---
        #darcy:2022/12/05 add s---
        on change imaud25
            if g_ima.imaud25 = 'Y' then
               call cl_set_comp_entry("imaud26",true)
            else
               call cl_set_comp_entry("imaud26",false)
               let g_ima.imaud26 = 'N'
               DISPLAY BY name g_ima.imaud26
            end if
        #darcy:2022/12/05 add e---

        AFTER FIELD imaud42
           IF g_ima.imaud42 IS NOT NULL THEN
              IF g_ima.imaud42 <0 THEN
                 CALL cl_err('','mfg4012',0)
                 NEXT FIELD imaud42
              END IF
           END IF
           ...
           AFTER FIELD imaud43
           IF g_ima.imaud43 IS NOT NULL THEN
              IF g_ima.imaud43 <0 THEN
                 CALL cl_err('','mfg4012',0)
                 NEXT FIELD imaud43
              END IF
           END IF
...
#DEV-D30026---add--end------------
        ...
        AFTER FIELD imaud07 
         ...
         if not cl_null(g_ima.imaud07) then
            if g_ima.ima06 ='G02' then
               if not aimi100_imaud07(g_ima.imaud07) then
                  next field imaud07
               end if
            end if
         end if
        ...
        AFTER INPUT  #判斷必要欄位之值是否有值,若無則反白顯示,並要求重新輸入
           ...
           LET g_ima.imagrup = s_get_data_group("ima_file") #FUN-C10039
            ...
            CASE i100_i_inpchk()
               ...
               when 'imaud25' next field imaud25 #darcy:2023/04/12 add
...
#                  g_ima.ima902 ,g_ima.ima37  ,g_ima.ima51  ,               #No.FUN-8C0131 
                  ...
                  #g_ima.ima1013,g_ima.ima1015,g_ima.ima1014,  #CHI-CA0073  mark 
                   ...
                   ,g_ima.imaud31,g_ima.imaud42,g_ima.imaud43
                    #darcy:2022/11/25 add
                   ,g_ima.imaud32 #darcy:2023/07/15 add
                   ,g_ima.imaud25,g_ima.imaud26 #darcy:2022/12/05 add
                   ,g_ima.imaud27,g_ima.imaud28  #darcy:2023/11/06
#No.FUN-A50011 -----begin-----
...
#  LET g_ima.ima262 = 0    #FUN-A20044 mark
   ...
   LET g_ima.ima71 = 0
   LET g_ima.imaud31 = 0 #darcy:2022/11/25 add
   LET g_ima.imaud32 = 0 #darcy:2023/07/15 add
   #darcy:2022/12/05 add s---
   let g_ima.imaud25 = 'N'
   let g_ima.imaud26 = 'N'
   let g_ima.imaud27 = 'N'
   call cl_set_comp_entry("imaud26",FALSE)
   #darcy:2022/12/05 add e---
   LET g_ima.imaud42 = 0 #xyb:2022/12/02 add
   LET g_ima.imaud43 = 0 #xyb:2022/12/02 add
   LET g_ima.ima72 = 0
...
#MOD-C30551 ----- add ----- end
   #darcy:2023/04/12 add s---
   if g_ima.imaud25 is null and (g_ima.ima06 ='G01' or g_ima.ima06 ='G02') then
      call cl_err('','cim-024',1)
      return "imaud25"
   end if
