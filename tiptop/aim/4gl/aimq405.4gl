...
#     DEFINE   l_time LIKE type_file.chr8     #No.FUN-6A0074
   ...
   END IF
    ...
    OPEN WINDOW aimq405_w AT p_row,p_col
         WITH FORM "aim/42f/aimq405"
...
#       CALL q405_q()
#    END IF
IF NOT cl_null(g_argv1) THEN CALL q405_q() END IF
    CALL q405_menu()
    CLOSE WINDOW q400_w
...
#No.TQC-650115--end   
  ...
  #MOD-860056-end-add
   ...
   PREPARE q405_prepare FROM g_sql
   DECLARE q405_cs                         #SCROLL CURSOR
           SCROLL CURSOR FOR q405_prepare
  ...
  #MOD-860056-end-add
   ...
   PREPARE q405_pp  FROM g_sql
   DECLARE q405_count   CURSOR FOR q405_pp
END FUNCTION
 
FUNCTION q405_b_askkey()
   CONSTRUCT tm.wc2 ON img01 FROM s_img[1].img01
              #No.FUN-580031 --start--     HCN
              BEFORE CONSTRUCT
                 CALL cl_qbe_init()
...
#str----end by huanglf160927
    ...
    OPEN q405_cs                            # 從DB產生合乎條件TEMP(0-30秒)
    IF SQLCA.sqlcode THEN
       CALL cl_err('',SQLCA.sqlcode,0)
    ELSE
       OPEN q405_count
       FETCH q405_count INTO g_row_count
       DISPLAY g_row_count TO FORMONLY.cnt
       CALL q405_fetch('F')                  # 讀出TEMP第一筆並顯示
    END IF
END FUNCTION
 
FUNCTION q405_fetch(p_flag)
DEFINE
    p_flag          LIKE type_file.chr1,     #處理方式    #No.FUN-690026 VARCHAR(1)
    l_abso          LIKE type_file.num10     #絕對的筆數  #No.FUN-690026 INTEGER
 
    CASE p_flag
        WHEN 'N' FETCH NEXT     q405_cs INTO g_imd.imd01 #MOD-860056 modify
        WHEN 'P' FETCH PREVIOUS q405_cs INTO g_imd.imd01 #MOD-860056 modify
        WHEN 'F' FETCH FIRST    q405_cs INTO g_imd.imd01 #MOD-860056 modify
        WHEN 'L' FETCH LAST     q405_cs INTO g_imd.imd01 #MOD-860056 modify
        WHEN '/'
      ...
      ON ACTION controlg      #MOD-4C0121
         CALL cl_cmdask()     #MOD-4C0121
               ...
               END IF
            END IF
            FETCH ABSOLUTE g_jump q405_cs INTO g_imd.imd01 #MOD-860056 modify
            LET mi_no_ask = FALSE
    END CASE
...
FUNCTION q405_b_fill()              #BODY FILL UP
  #DEFINE l_sql     LIKE type_file.chr1000 #No.FUN-690026 VARCHAR(1000)   #MOD-B90146 mark
   DEFINE l_sql,l_sql1,l_sql2     STRING                                                #MOD-B90146
    ...
    #darcy:2023/07/04 add s---
    # 最近请购人
    let l_str = "SELECT rvv31,rvv32,pmk12,pmk13,pmk04 FROM (",
                " SELECT rvv31,rvv32,pmk12,pmk13,rvu03,pmk04, ",
                " dense_rank() OVER(PARTITION BY rvv31,rvv32 ORDER BY rvu03 DESC,pmk04 DESC ) r",
                " FROM rvv_file ,rvu_file ,",
                " pmn_file,pml_file,pmk_file",
                " WHERE rvv01 =rvu01",
                " AND rvuconf='Y'",
 ...
 IF g_imd.chk1 = 'N' THEN 
   LET l_sql =
        ...
        " ,pmk12,gen02,pmk13,gem02,pmk04 ",#darcy:2023/07/04 add
       #" ,ima02, ima021",          #FUN-640006  #MOD-6A0130 mark
        " FROM  img_file",
        "   left join (",l_str,") on rvv31=img01 and rvv32=img02", #darcy:2023/07/04 add
        "   left join gen_file on gen01 =pmk12 ",
        "   left join gem_file on gem01 =pmk13 ",
        " , ima_file",  #FUN-640006   #No.TQC-780080 add
        " WHERE img02 = '",g_imd.imd01,"' AND ", tm.wc2 CLIPPED,
        " AND   img_file.img01 = ima_file.ima01 AND img10!=0",   #FUN-640006
        " ORDER BY img01 "
    PREPARE q405_pb FROM l_sql
    DECLARE q405_bcs                       #BODY CURSOR
        CURSOR FOR q405_pb
ELSE 
   ...
   LET l_sql1 =
       ...
       #" ,ima02, ima021",          #FUN-640006  #MOD-6A0130 mark
        ...
        " ORDER BY img01 "
    PREPARE q405_pb1 FROM l_sql1
    DECLARE q405_bcs1                       #BODY CURSOR
        CURSOR FOR q405_pb1
END IF 
   ...
   #---------------No:MMOD-810252 end
    ...
    LET g_cnt = 1
IF g_imd.chk1 = 'N' THEN 
    FOREACH q405_bcs INTO g_img[g_cnt].*
        IF SQLCA.sqlcode THEN
...
#       END IF
    ...
    END FOREACH
ELSE
     FOREACH q405_bcs1 INTO g_img[g_cnt].*
        IF SQLCA.sqlcode THEN
            CALL cl_err('Foreach:',SQLCA.sqlcode,1)
            EXIT FOREACH
        END IF
    ...
    PREPARE q405_pb2 FROM l_sql2
    EXECUTE q405_pb2 INTO g_img[g_cnt].img09,g_img[g_cnt].img19,g_img[g_cnt].img36,g_img[g_cnt].img37,g_img[g_cnt].img18
#--TQC-CC0003--add----star---
        SELECT ime03  INTO g_img[g_cnt].ime03 FROM ime_file WHERE ime02 = g_img[g_cnt].img03 
                AND ime01 = g_imd.imd01  #add by guanyao160802
        DISPLAY g_img[g_cnt].ime03 TO ime03
