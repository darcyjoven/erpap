...
#              "ima262.ima_file.ima262,",   #FUN-A40023
               ...
               ,",img18_2.img_file.img18,",
               "l_day_2.type_file.num10,",
               "imaud31.ima_file.imaud31"
   ...
   LET g_sql = "INSERT INTO ",g_cr_db_str CLIPPED,l_table CLIPPED,
               " VALUES(?,?,?,?,?,  ?,?,?,?,?,   ?,?,?,?,?,  ?,?,?,?,?,",
               "        ?,?,?,?,?   ,?,?,?,?,?, ?,?,?,?,?)"          	     #---TQC-CC0074--add-- #darcy:2022/11/25 add 3?
...
################################################################################
   ...
   LET tm.s      = '12'
   # LET tm.f      = '1'
   LET tm.f      = '3' #darcy:2024/03/05 mod '1' -> '3'
   LET tm.g      = '1'
   LET tm.h      = 'Y'
   LET tm.b      = 'Y'
   LET tm.c      = 'Y'
...
#                    ima262   LIKE ima_file.ima262,#庫存可用數量     #FUN-A40023
    		     ...
    		     ima86    LIKE  ima_file.ima86, #成本單位    #No.MOD-6A0043 add
                     ...
                     #darcy:2022/11/25 add s---
                     ,img18_2 like img_file.img18,
                     imaud31  like ima_file.imaud31 
                     #darcy:2022/11/25 add e---
                     END RECORD
DEFINE    l_day,l_day_2      LIKE type_file.num10   #No.FUN-840055  #darcy:2022/11/25 add  l_day_2
   ...
   define l_last_yy,l_last_mm  integer #darcy:2024/03/05 add
     #No.FUN-840055-------start--
     CALL cl_del_data(l_table)

     #darcy:2024/03/05 add s---
     #   取上月期别
     select ccz01,ccz02 into l_last_yy,l_last_mm from ccz_file
   #   let l_last_yy = YEAR(g_today)
   #   let l_last_mm = MONTH(g_today)
   #   if l_last_mm = 1 then
   #    let l_last_yy = l_last_yy -1
   #    let l_last_mm = 12
   #   else
   #    let l_last_mm = l_last_mm - 1
   #   end if
...
#				 " ima262,ima53,ima531,ima86",      #FUN-A40023
                              ...
                              #---------No.MOD-6A0043 end
                 " ,img13+NVL(imaud31,0) img18_2,nvl(imaud31,0)", #darcy:2022/11/25 add
...
#No.FUN-590110 --end--
     ...
     if tm.f = '3' then
      #   
      let sr.cost = 0
      select ccc23 into sr.cost from ccc_file 
         where ccc02 = l_last_yy and ccc03 = l_last_mm
         and ccc01 = sr.ima01
      if cl_null(sr.cost) or sr.cost = 0 then
         let sr.cost = sr.ima53
      end if
     else
     #darcy:2024/03/05 add e---
...
#No.FUN-590110 --end--
       IF sr.delay >= tm.e AND sr.delay < tm.i THEN    #介於天數區間
          ...
          END IF 
          #darcy:2022/11/25 add s---
          if cl_null(sr.img18_2)  or sr.img18_2=mdy(12,31,9999) then 
            let l_day_2 ='' 
          else 
            let l_day_2 = sr.img18_2 - g_today   #有效截止天數
          end if 
          #darcy:2022/11/25 add e---
...
#             sr.img09,sr.img10,sr.cost,sr.ima25,sr.ima262,sr.ima73,sr.ima74,   #FUN-A40023
              ...
              sr.ima71,sr.bz
              ,sr.img18_2,l_day_2,sr.imaud31 #darcy:2022/11/25 add
