...
GLOBALS "../../config/top.global"
   ...
   DEFINE g_rvu          DYNAMIC ARRAY OF RECORD  
    ...
    DEFINE g_rvv          DYNAMIC ARRAY OF RECORD  
                ...
                ta_rvw01    like rvw_file.ta_rvw01,  #darcy:2023/06/26
      ...
      CASE g_action_choice
         ...
         when "fastexcel"
            if cl_chk_act_auth() then
               if cl_chk_act_auth() then
                  call list.clear()
                  let list[1].node = base.typeinfo.create(g_rvv_excel)
                  let list[1].name = "s_rvv"
...
DEFINE l_wc       STRING
   ...
   LET l_sql = " UPDATE apmq991_tmp ",
               "    SET rvv38_1 = rvv38 * nvl(pmm42,1),",
               "        rvv39_1 = rvv39 * nvl(pmm42,1),",
               "        rvv39t_1= rvv39t * nvl(pmm42,1),",
               "        rvv39t_2= rvv39t * nvl(pmm42,1)"
   ...
   #报关单号
   let l_sql = "MERGE INTO apmq991_tmp a USING(",
               " SELECT rvw08,rvw09,MAX(ta_rvw01) ta_rvw01 FROM rvw_file",
               " WHERE ta_rvw01 IS NOT NULL GROUP BY rvw08,rvw09)b",
               " ON(a.rvu01=b.rvw08 AND a.rvv02=b.rvw09)",
               " WHEN MATCHED THEN UPDATE SET a.ta_rvw01=b.ta_rvw01"
   PREPARE q991_pre100 FROM l_sql
   EXECUTE q991_pre100
