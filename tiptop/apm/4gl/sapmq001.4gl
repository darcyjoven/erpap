...
GLOBALS "../../../tiptop/config/top.global"
   ...
   DEFINE g_pmk,g_pmk_excel    DYNAMIC ARRAY OF RECORD  #48
                ...
                pml06     like pml_file.pml06     #darcy:2023/11/27 add
                         END RECORD 
...
FUNCTION q001_menu()
   ...
   WHILE TRUE
      ...
      CASE g_action_choice
         ...
         #darcy:2023/12/21 add s---
         when "fastexcel"
            #darcy:2024/01/18 add s---
            if g_user = 'tiptop' then 
               call cl_expexcel_display(
                     "s_pmk",base.typeinfo.create(g_pmk_excel),
                     "s_pmk_1",base.typeinfo.create(g_pmk_1),
                     "",""
               ) 
            end if
            #darcy:2024/01/18 add e---
            if cl_chk_act_auth() then
               call list.clear()
               let list[1].node = base.typeinfo.create(g_pmk_excel)
               let list[1].name = "s_pmk"
               let list[2].node = base.typeinfo.create(g_pmk_1)
               let list[2].name = "s_pmk_1"
   ...
   CASE g_pname 
      WHEN '2'
         ...
         LET l_sql ="SELECT '' speed,'' state,pmm01 pmk01,pmm02 pmk02,pmm04 pmk04,trim(nvl(pmm12,'')) pmk12,gen02,trim(nvl(pmm13,'')) pmk13,gem02,trim(nvl(pmm09,'')) pmk09,", 
                    ...
                    "                left join pml_file on pml01 =pmn24 and pml02 = pmn25 ", #darcy:2023/11/27 add
