...
#               IF (g_sgr.sgr01 != g_sgr01_t) OR (g_sgr01_t IS NULL) THEN  #FUN-A50100 
               ...
               if not i120_chk_release(g_sgr.sgr01,g_sgr.sgr02) then
                  next field sgr02
               end if
...
function i120_chk_release(p_ecu01,p_ecu02)
   ...
   let l_cnt = 0
   select count(*) into l_cnt from sfb_file 
    where sfb05 = p_ecu01 and sfb06 = p_ecu02 and sfb87 != 'X'

   if l_cnt > 0 then
      call cl_err(p_ecu01,"csf-119",1)
      return false
   end if
   return true
end function
