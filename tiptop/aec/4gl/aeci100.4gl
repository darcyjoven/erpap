...
#   g_ecu      RECORD LIKE ecu_file.*,    #FUN-A50100 移至aecp110.global
    ...
    g_ecb           DYNAMIC ARRAY OF RECORD    #程式變數(Program Variables)
        selected        like type_file.chr1,   #darcy:2024/08/13 add
...
#FUN-B90117--END--
         ...
         #darcy:2023/03/29 add s---
         when "copy_y"
            if cl_chk_act_auth() then
               call s_showmsg_init()
               call i100_copy_y(g_ecu.ecu01,g_ecu.ecu02)
               call s_showmsg()
            end if
         ...
         when "run_cecp200"
            if cl_chk_act_auth() then
               let g_cmd = "cecp200 '",g_ecu.ecu01,"' '",g_ecu.ecu02,"' 'Y' 'Y' 'N'"
               call cl_cmdrun(g_cmd)
               call i100_show()
            end if
         #darcy:2023/04/19 add e---
         #darcy:2023/07/19 add s---
         when "cecp300"
            if cl_chk_act_auth() then
               let g_cmd = "cecp300 '",g_ecu.ecu01,"' '' 'Y' 'N' "
               call cl_cmdrun(g_cmd)
               call i100_show()
            end if
...
#                     call i100sub_upd_bmb09(g_ecu.ecu01,g_ecu.ecu02) #darcy:2023/06/15 add
#                     LET g_ecu.ecuud02="Y"
...
#darcy:2023/10/17 mark e---
            ...
            call i100sub_y_chk(g_ecu.ecu01,g_ecu.ecu02)
            if g_success = 'Y' then
               call i100sub_y_upd(g_ecu.ecu01,g_ecu.ecu02,false)
            end if
            if g_success = 'Y' then
               let g_ecu.ecuud02 = 'Y'
            end if
      ...
      IF cl_chk_act_auth() THEN
       ...
       WHEN "upecbud06"
            ...
            # if g_user = 'tiptop' then
               foreach i100_ecb_cl using g_ecu.ecu01,g_ecu.ecu02 into b_ecb.*
                  # select * into b_ecb.* from ecb_file where ecb01 = g_ecu.ecu01 and ecb02 = g_ecu.ecu02
                  if not sqlca.sqlcode then
                     if not i100sub_mod_log(b_ecb.*,"update") then
                        call cl_err("插入变更记录失败",'!',1)
                     end if
                  end if
               end foreach
            # end if
...
FUNCTION i100_a()
    ...
    LET g_ecu.ecuud02 = 'N'
    let g_ecu.ecuud10 = '0'  #darcy:2023/12/04 add
        ...
        AFTER FIELD ecuud10
         ...
         on change ecuud10
            if g_ecu.ecuud10 = '1' then
               call cl_set_comp_entry("ecuud11",true)
               call cl_set_comp_required("ecuud11",true)
            else
               call cl_set_comp_entry("ecuud11",false)
            end if
         #darcy:2023/12/04 add e---
...
#FUN-AA0059 --End--
   ...
   # 更新组装报工和修改记录
   # if g_user = 'tiptop' then
      let l_ima06 = cs_get_ima06(new_no)
      if l_ima06 = 'G01' then
         if not i100sub_upd_G01(new_no,necu02) then
            call cl_err("更新报工否失败",'!',1)
         end if
      # end if
      foreach i100_ecb_cl using new_no,necu02 into b_ecb.*
         # select * into b_ecb.* from ecb_file where ecb01 = new_no and ecb02 =necu02
         if not sqlca.sqlcode then
            if not i100sub_mod_log(b_ecb.*,"insert") then
               call cl_err("插入变更记录失败",'!',1)
            end if
         end if
      end foreach
...
define l_ima06    like ima_file.ima06 #darcy:2024/03/05 add
#darcy:2024/08/13 add s---
define l_i        integer
define l_delete   like type_file.chr1
#darcy:2024/08/13 add e---
    ...
    LET g_forupd_sql =
        "SELECT 'N',ecb03,ecb06,ecb17,ecb08,' ','',ecb07,'',ecb38,ecb04,ecb19,ecb18,", #darcy:2024/08/13 add 'N'
...
#       "       ecb42,ecb43,ecb44,ecb45,ecb46,ecb48,ecb49",                      #No.FUN-810017 #FUN-960056  #FUN-A50081
    ...
    INPUT ARRAY g_ecb WITHOUT DEFAULTS FROM s_ecb.*
        ...
        BEFORE INSERT
            ...
            LET g_ecb[l_ac].selected = 'N' #darcy:2024/08/13 add
...
#                                g_ecb[l_ac].ecb44,g_ecb[l_ac].ecb45,  #FUN-A50081
            ...
            ELSE
               ...
               # 更新组装报工和修改记录
               # if g_user = 'tiptop' then
                  let l_ima06 = cs_get_ima06(g_ecu.ecu01)
                  if l_ima06 = 'G01' then
                     if not i100sub_upd_G01(g_ecu.ecu01,g_ecu.ecu02) then
                        call cl_err("更新报工否失败",'!',1)
                     end if
                  end if
                  select * into b_ecb.* from ecb_file
                   where ecb01 = g_ecu.ecu01 and ecb02 =g_ecu.ecu02
                     and ecb06 = g_ecb[l_ac].ecb06
                  if not sqlca.sqlcode then
                     if not i100sub_mod_log(b_ecb.*,"insert") then
                        call cl_err("插入变更记录失败",'!',1)
                     end if
        ...
        BEFORE DELETE                            #是否取消單身
            ...
            for l_i = 1 to g_ecb.getLength()
               if g_ecb[l_i].selected = 'Y' then
                  ...
                  if not aeci100_delete_ecb(g_ecu.ecu01,g_ecu.ecu02,g_ecb[l_i].ecb03,l_lock_sw) then
                     rollback work
                     cancel delete
                     exit for
                  end if
...
FUNCTION i100_set_entry_b(p_cmd)
   ...
   if l_ac > 0 and l_ac <=  g_ecb.getLength() then
      ...
      if g_user = '50863' and g_ecb[l_ac].ecb06 matches "[FM]*" then
         # 光板：50863余阁
         call cl_set_comp_entry("ecbud06",true)
      end if
      if (g_user = '55681' or g_user = '55759') and g_ecb[l_ac].ecb06 matches "Z*" then
         # 载板：55681孙万祥 55759周转转
         call cl_set_comp_entry("ecbud06",true)
      end if
   end if
   #darcy:2023/07/08 add e---
   #darcy:2024/03/06 add s---
   # 设置组装料号只有g01权限才能修改报工否
   let l_ima06 = cs_get_ima06(g_ecu.ecu01)
   if l_ima06 = 'G01' then
...
FUNCTION i100_set_no_entry_b(p_cmd)
  ...
  #darcy:2024/03/06 add e---
   ...
   # 设置载板和光板可以修改报工否时，只能修改自己的报工
   if l_ac > 0 and l_ac <=  g_ecb.getLength() then
      if g_user = '50863' and g_ecb[l_ac].ecb06 matches "Z*" then
         # 光板：50863余阁
         call cl_set_comp_entry("ecbud06",false)
      end if
      if (g_user = '55681' or g_user = '55759') and g_ecb[l_ac].ecb06 matches "[FM]*" then
         # 载板：55681孙万祥 55759周转转
         call cl_set_comp_entry("ecbud06",false)
      end if
   end if
...
#        ACCEPT DISPLAY                   #No.FUN-530067 HCN TEST    #FUN-B90117
      ...
      #整批复制，根据BOM复制
      on action copy_y
         let g_action_choice = 'copy_y'
         exit dialog

      #darcy:2023/02/17 e---
      #darcy:2023/04/19 add s---
      on action run_cecp200
         let g_action_choice = 'run_cecp200'
         exit dialog
      #darcy:2023/04/19 add e---
      #darcy:2023/07/19 add s---
      on action cecp300
         let g_action_choice = "cecp300"
         exit dialog
      #darcy:2023/07/19 add e---
      #darcy:2024/03/06 add s---
      on action g01
...
#str----end by huanglf160927
      ...
      #darcy:2023/02/14 s---
      on action upd_ecbud06
         let g_action_choice = "upd_ecbud06"
         if g_ecu.ecu10 != 'Y' then
            if cl_chk_act_auth() then
               call i100_upd_ecbud06(g_ecu.ecu01,g_ecu.ecu02)
            end if
         else
            message "已发出，不允许更新！"
         end if
      ...
      IF iRes <> -1 THEN
         ...
         FOR i=2 TO iRow                           
            ...
            FOR j=i TO l_y
               ...
               # 更新组装报工和修改记录
               # if g_user = 'tiptop' then
                  let l_ima06 = cs_get_ima06(l_ecu.ecu01)
                  if l_ima06 = 'G01' then
                     if not i100sub_upd_G01(l_ecu.ecu01,l_ecu.ecu02) then
                        call cl_err("更新报工否失败",'!',1)
                     end if
                  end if
...
DEFINE g_ecbud04   LIKE ecb_file.ecbud04   
    ...
    LET g_success = 'Y'
      ...
      if not cl_null(g_ecb_t.ecb06) then
         let l_sql = l_sql , " and ecb06 <> '",g_ecb_t.ecb06,"'"
      end if
      let l_sql = l_sql,
...
#darcy:2022/11/23 add s---
# 更新是否报工
function i100_upd_ecbud06(p_ecb01,p_ecb02)
   define p_ecb01    like ecb_file.ecb01
   define p_ecb02    like ecb_file.ecb02
   define l_sql      string
   define l_ecb      dynamic array of record
   ...
   define l_ecb03    dynamic array of integer  #特殊情况的开始，和结束位置
   define l_flag_c   like type_file.chr1
   define l_flag     like type_file.chr1
   define l_cnt      like type_file.num5
   define l_index    integer
   define l_index2   integer
   define l_index3   integer
   define l_index4   integer
   define l_ecb08_p  like ecb_file.ecb08 # 上一笔工作站
   define l_ta_ecd09 like ecd_file.ta_ecd09
   ...
   let l_sql = "UPDATE ecb_file 
                  ...
                  AND ecb06 IN (
                  SELECT ecd01 FROM ecd_file WHERE ta_ecd09=1
                  )"
   prepare ecbud06_p1 from l_sql

   # 特殊情况抓取
   let l_sql = "select ta_ecd09 from ecd_file where ecd01 = ? "
   prepare ecbud06_p2 from l_sql

   # 遍历所有
   let l_sql = "SELECT ecb01,ecb02,ecb03,ecb06,ecb08,ecbud06 FROM ecb_file WHERE ecb01 = ? and ecb02 = ? order by ecb01,ecb02,ecb03"
   prepare ecbud06_p3 from l_sql
   declare ecbud06_c3 cursor for ecbud06_p3

   # 获取报工类型和说明
   let l_sql = "select ecd02, ta_ecd09 from ecd_file where ecd01 = ?"
   prepare ecbud06_p4 from l_sql

   # 主制程更新 s---
   # execute ecbud06_c1 using p_ecb01,p_ecb02
   # if status then
   #    call cl_err('ecbud06_c1',sqlca.sqlcode,1)
   #    return
   # end if
   # 主制程更新 e---
   let l_success = 'Y'
   ...
   update ecb_file set ecbud06 = 'N' where ecb01 =p_ecb01 and ecb02 =p_ecb02
   if status then
      call cl_err('upd ecb_file',sqlca.sqlcode,1)
      let l_success = 'N'
      return
   end if
   # 全部更新为不勾选 e---
   ...
   # 记录修改记录
   # if g_user = 'tiptop' then
      foreach i100_ecb_cl using p_ecb01,p_ecb02 into b_ecb.*
      # select * into b_ecb.* from ecb_file where ecb01 = p_ecb01 and ecb02 = p_ecb02
         if not sqlca.sqlcode then
            if not i100sub_mod_log(b_ecb.*,"update") then
               call cl_err("插入变更记录失败",'!',1)
            end if
   ...
   # 遍历1 s---
   let l_index = 1
   foreach ecbud06_c3 using p_ecb01,p_ecb02 into l_ecb[l_index].*
      if status then
         call cl_err('ecbud06_c3',sqlca.sqlcode,1)
         let l_success = 'N'
         exit foreach
      end if
      let l_index = l_index + 1
   end foreach
   call l_ecb.deleteElement(l_index)
   # 遍历1 e---

   # 遍历2 s---
   # 左游标l_index2 右游标l_index
   let l_index2 = 1
   for l_index = 1 to l_ecb.getLength()
      # 检查跨站
      if l_ecb[l_index].ecb08 != l_ecb[l_index2].ecb08 and l_ecb[l_index].ecb08 !='G1018' then
         # 如果跨站，检查是否有报工
         let l_flag = 'N'
         for l_index3 = l_index2 to l_index
            if l_ecb[l_index3].ecbud06 ='Y' then
               let l_flag = 'Y'
               exit for
            end if
         end for
         if l_flag = 'N' then
            # 需要勾选第一站为报工
            let l_ecb[l_index2].ecbud06 = 'Y'
         end if
         # 如果时最后一个站别还需勾选一次报工
         # 此站别勾选完工，继续下一个站别
         let l_index2 = l_index
      end if
      ...
      case l_ta_ecd09
         when 1 # 主制程,勾选后继续
            let l_ecb[l_index].ecbud06 = 'Y'
         # when 2 # 其它情况，不需处理
         when 4 # 连续的模切裁板-辅材和模切加工-辅材，只勾选第一笔的报工。
            let l_ecb[l_index].ecbud06='Y'
            for l_index3 = l_index + 1 to l_ecb.getLength()
               # 如果后续还是特殊情况1,右游标右移
               if l_ecb[l_index3].ecb08 != l_ecb[l_index2].ecb08 and l_ecb[l_index3].ecb08 !='G1018' then
                  # 如果跨站,左右游标设置为当前位置,跳出遍历
                  # 不需要检查是否报工,因为辅材第一站必定报工
                  let l_index2 = l_index3
                  let l_index = l_index3 - 1
                  exit for
               end if
               execute ecbud06_p2 using l_ecb[l_index3].ecb06 into l_ta_ecd09
               if status then
                  call cl_err("ecbud06_p2",sqlca.sqlcode,1)
                  let l_success = 'N'
               end if
               if l_ta_ecd09 = 4 then
         ...
         when 6 # 连续的电镀-图形镀铜和电镀-图形去膜，报工勾选镀铜，不勾选去膜。
            #  从当前位置l_index,而不是l_index + 1
            for l_index3 = l_index to l_ecb.getLength()
               if l_ecb[l_index3].ecb08 != l_ecb[l_index2].ecb08 and l_ecb[l_index3].ecb08 !='G1018'then
                  #跨站需要检查报工
                  let l_flag = 'N'
                  for l_index4 = l_index2 to l_index3
                     if l_ecb[l_index4].ecbud06 ='Y' then
                        let l_flag = 'Y'
                        exit for
                     end if
                  end for
               ...
               execute ecbud06_p4 using l_ecb[l_index3].ecb06 into l_ecd02,l_ta_ecd09
               if status then
                  call cl_err('ecbud06_p4',sqlca.sqlcode,1)
                  let l_success = 'N'
                  exit for
               end if
               if l_ta_ecd09 = 6 then
                  # 特殊情况
                  if l_ecd02 matches '*图形镀铜*' then
                     let l_ecb[l_index3].ecbud06 = 'Y'
                  end if
                  let l_index = l_index3
               else
         ...
         when 7 # 连续的冲切-辅料分割和冲切-辅材，优先勾选冲切-辅材。
                # 冲切辅料连带只报一个，有辅材时优先选辅材，无辅材时优先选成型
            # 和电镀-图形一样同当前位置遍历
            let l_flag7 = 'N' #darcy:2022/12/31 add
            for l_index3 = l_index to l_ecb.getLength()
               if l_ecb[l_index3].ecb08 != l_ecb[l_index2].ecb08 and l_ecb[l_index3].ecb08 !='G1018' then
                  #跨站需要检查报工
                  let l_flag = 'N'
               ...
               if l_ta_ecd09 = 7 then
                  ...
                  # 其次选择成型
                     ...
                     if l_ecd02 matches '*成型*' and l_flag7 ='N' then
                        let l_ecb[l_index3].ecbud06 = 'Y'
                        let l_flag7 = 'Y'
                        #TODO:勾选一次就不再勾选了。
                     end if
                  #darcy:2022/12/31 add e---
                  end if
                  let l_index = l_index3
               else
                  # 不是特殊情况,就退出遍历
         ...
         when 8 # 连续的图形-曝光,显影-压膜和图形-显影，勾选图形-显影,显影-压膜.
            for l_index3 = l_index to l_ecb.getLength()
               if l_ecb[l_index3].ecb08 != l_ecb[l_index2].ecb08 and l_ecb[l_index3].ecb08 !='G1018' then
                  #跨站需要检查报工
                  let l_flag = 'N'
               ...
               if l_ta_ecd09 = 8 then
                  # 特殊情况
                  if l_ecd02 matches '*显影*' then
                     let l_ecb[l_index3].ecbud06 = 'Y'
                  end if
                  if l_ecd02 matches '*压膜*' then
                     let l_ecb[l_index3].ecbud06 = 'Y'
                  end if
                  let l_index = l_index3
               else
      ...
      # 最后一个工作站,但还是没有报工
      # 勾选最后一工作站,第一个作业编号的报工.
      if l_ecb[l_index2].ecb08 =l_ecb[l_index].ecb08
         and l_index = l_ecb.getLength()
      then
         let l_flag = 'N'
         for l_index3 = l_index2 to l_index
            if l_ecb[l_index3].ecbud06 = 'Y' then
      ...
      # let l_ecb08_p = l_ecb[l_index].ecb08
   end for
   # 遍历2 e---
   ...
   for l_index = 1 to l_ecb.getLength()
      if l_ecb[l_index].ecbud06 = 'Y' then
         ...
         # if g_user = 'tiptop' then
            select * into b_ecb.* from ecb_file
             where ecb01 = l_ecb[l_index].ecb01 and ecb02 = l_ecb[l_index].ecb02
               and ecb06 = l_ecb[l_index].ecb06
            if not sqlca.sqlcode then
               if not i100sub_mod_log(b_ecb.*,"update") then
                  call cl_err("插入变更记录失败",'!',1)
               end if
...
function i100_copy_y(p_ecu01,p_ecu02)
   define p_ecu01  like ecu_file.ecu01
   define p_ecu02  like ecu_file.ecu02
   DEFINE new_no,old_no   LIKE ecu_file.ecu01,
           oecu02,necu02   LIKE ecu_file.ecu02,
...
#FUN-AA0059 --End--
   ...
   #darcy:2023/03/29 add s---
   # drop临时表
   # drop table时会提交事务，所以drop table改为delete table
   # drop table 放在事务外处理
   DROP TABLE ecu_tmp
   let l_sql = "select * from ecu_file where 1=2 into temp ecu_tmp"
   prepare i100_ecu_crt from l_sql
   execute i100_ecu_crt
   DROP TABLE ecb_tmp
   let l_sql = "select * from ecb_file where 1=2 into temp ecb_tmp"
   prepare i100_ecb_crt from l_sql
   execute i100_ecb_crt
   DROP TABLE sgc_tmp
   let l_sql = "select * from sgc_file where 1=2 into temp sgc_tmp"
   prepare i100_sgc_crt from l_sql
   execute i100_sgc_crt
   #darcy:2023/03/29 add e---
   ...
   IF STATUS THEN
      # CALL cl_err("OPEN i100_cl:", STATUS, 1)
      CALL s_errmsg("","","OPEN i100_cl:",STATUS,1)
      CLOSE i100_cl
      ROLLBACK WORK
      RETURN
   END IF
   ...
   IF STATUS THEN
      # CALL cl_err3("ins","sgc_file",new_no,necu02,STATUS,"","ins sgc",1) #FUN-660091
      CALL s_errmsg("",new_no||necu02,'ins sgc',STATUS,1)
   RETURN END IF
   #FUN-C30027---begin
   #darcy:2023/03/29 add s---

   let l_sql = "SELECT bmb03 FROM bmb_file,bma_file
                WHERE bmb01 = ? AND bmb01 =bma01  AND bma10 ='2'
                AND bmb04 <=SYSDATE AND (bmb05 >SYSDATE OR bmb05 IS NULL)
                AND bmb03 NOT LIKE '%.%'"
   prepare i100_bom_p from l_sql
   declare i100_bom_c cursor for i100_bom_p


   call i100_bom(old_no,oecu02,oecu012,new_no,necu02,necu012)
   if g_success = 'N' then
      rollback work
   else
   #darcy:2023/03/29 add e---
...
function i100_bom(old_no,oecu02,oecu012,new_no,necu02,necu012)
   define old_no  like bmb_file.bmb01,
          oecu02  like ecu_file.ecu02,
          oecu012 like ecu_file.ecu012,
          new_no  like bmb_file.bmb01,
          necu02  like ecu_file.ecu02,
          necu012 like ecu_file.ecu012

   define l_sql   string

   define l_new_no        like bmb_file.bmb01
   define l_flag  like type_file.chr1
   define l_state like type_file.chr10
   define l_cnt   like type_file.num5

   define l_bmb03 dynamic array of varchar(40)
   define l_i     like type_file.num5
   ...
   foreach i100_bom_c using old_no into l_bmb03[l_i] 
      if status then
         ...
         CALL s_errmsg("",new_no||necu02,'i100_bom_c',STATUS,1)
         exit foreach
      end if
      let l_i = l_i + 1
   end foreach
   call l_bmb03.deleteElement(l_i)

   for l_i = 1 to l_bmb03.getLength()
      # 新料件编号
      let l_new_no = i100_new_item_no(l_bmb03[l_i],new_no)
      #NOTE: 是否可复制检查
      #1. 检查aimi100是否建立，报错继续
      select imaacti into l_flag from ima_file where ima01 = l_new_no
      if cl_null(l_flag) then
         let l_flag = 'N'
      end if
      if l_flag <>'Y' then
         call s_errmsg("ima01,imaacti",l_new_no||","||l_flag,'','cec-029',1)
         let g_success = 'N'
         continue for
      end if
      ...
      if cl_null(l_state) then
         let l_state = 'N'
      end if
      if l_flag <> 'Y' or l_state <> 'Y' then
         continue for
      end if
      ...
      if l_cnt > 0 then
         call s_errmsg("ecu01,ecu02,ecu012",l_new_no||","||necu02||","||necu012,'','cec-035',1)
         let g_success = 'N'
         continue for
      end if
      #NOTE: 开始复制
      #4. 锁住ecu，防止内容被修改
      open i100_cl using old_no,oecu02,oecu012
      if status then
         call cl_err("open i100_cl:", status, 1)
         call s_errmsg("ecu01,ecu02,ecu012",old_no||","||oecu02||","||oecu012,'',status,1)
         close i100_cl
         let g_success = 'N'
         rollback work
         continue for
      end if
      fetch i100_cl into g_ecu.*
      if sqlca.sqlcode then
         #  call cl_err('lock ecu:',sqlca.sqlcode,0)
         call s_errmsg("ecu01,ecu02,ecu012",old_no||","||oecu02||","||oecu012,'lock ecu:',sqlca.sqlcode,1)
         close i100_cl
         let g_success = 'N'
         rollback work
         continue for
      ...
      if not i100_copy_ecu(l_bmb03[l_i],oecu02,oecu012,l_new_no,necu02,necu012) then
         close i100_cl
         continue for
      end if
      if not i100_copy_ecb(l_bmb03[l_i],oecu02,oecu012,l_new_no,necu02,necu012) then
         close i100_cl
         continue for
      end if
      if not i100_copy_sgc(l_bmb03[l_i],oecu02,oecu012,l_new_no,necu02,necu012) then
         close i100_cl
         continue for
      end if
      ...
      if l_cnt > 0 then
         call i100_bom(l_bmb03[l_i],oecu02,oecu012,i100_new_item_no(l_bmb03[l_i],new_no),necu02,necu012)
      end if
...
function i100_new_item_no(p_ima01,p_newno)
   define p_ima01,p_newno like bmb_file.bmb01
   define l_new_no        like bmb_file.bmb01

   let l_new_no = cl_replace_str(p_ima01,p_ima01[1,10],p_newno[1,10])

   return l_new_no
end function

#NOTE: 复制ecu
function i100_copy_ecu(old_no,oecu02,oecu012,new_no,necu02,necu012)
   define old_no  like bmb_file.bmb01,
          oecu02  like ecu_file.ecu02,
          oecu012 like ecu_file.ecu012,
          new_no  like bmb_file.bmb01,
   ...
   if status then
      call s_errmsg("ecu01,ecu02,ecu012",new_no||","||necu02||","||necu012,'ins ecu',STATUS,1)
      let g_success = 'N'
      return false
   end if
...
#NOTE: 复制ecb
function i100_copy_ecb(old_no,oecu02,oecu012,new_no,necu02,necu012)
   define old_no  like bmb_file.bmb01,
          oecu02  like ecu_file.ecu02,
          oecu012 like ecu_file.ecu012,
          new_no  like bmb_file.bmb01,
   ...
   let l_sql = "insert into ecb_tmp ",
               "select * from ecb_file ",
               " where ecb01 = ? and ecb02 =? and ecb012 = ? "
   prepare i100_ecb_p from l_sql
   execute i100_ecb_p using old_no,oecu02,oecu012

   update ecb_tmp set ecb01=new_no,ecb02=necu02,ecb012=necu012
...
#NOTE: 复制sgc
function i100_copy_sgc(old_no,oecu02,oecu012,new_no,necu02,necu012)
   define old_no  like bmb_file.bmb01,
          oecu02  like ecu_file.ecu02,
          oecu012 like ecu_file.ecu012,
          new_no  like bmb_file.bmb01,
   ...
   let l_sql = " insert into sgc_tmp ",
               "select * from sgc_file ",
               " where sgc01= ? and sgc02 =? and sgc012 = ? "
   prepare i100_sgc_p from l_sql
   execute i100_sgc_p using old_no,oecu02,oecu012

   if status then
      call s_errmsg("ecu01,ecu02,ecu012",old_no||","||oecu02||","||oecu012,'ins sgc',STATUS,1)
      let g_success = 'N'
      return false
   end if

   update sgc_tmp set sgc01 =new_no,sgc02=necu02,sgc012=necu012

   insert into sgc_file select * from sgc_tmp
   if status then
      call s_errmsg("ecu01,ecu02,ecu012",new_no||","||necu02||","||necu012,'ins sgc',STATUS,1)
      let g_success = 'N'
      return false
   end if
...
#darcy:2024/08/13 add s---
function aeci100_delete_ecb(p_ecb01,p_ecb02,p_ecb03,p_lock)
   define p_ecb01    like ecb_file.ecb01,
          p_ecb02    like ecb_file.ecb02,
          p_ecb03    like ecb_file.ecb03
   define p_lock     like type_file.chr1

   if p_ecb03 is not null then
      if p_lock = "Y" then
         call cl_err("", -263, 1)
         return false
      end if
      ...
      delete from ecb_file
      where ecb01 = p_ecb01 and ecb02 = p_ecb02
         and ecb03 = p_ecb03
         and ecb012 = g_ecu.ecu012
      if sqlca.sqlcode then
         call cl_err3("del","ecb_file",p_ecb01,p_ecb02,sqlca.sqlcode,"","",1)
         rollback work
         return false
      end if

      delete from vms_file
         where vms01 = p_ecb01 and vms02 = p_ecb02
         and vms03 = p_ecb03

      if sqlca.sqlcode then
         call cl_err3("del","vms_file",p_ecb01,p_ecb02,sqlca.sqlcode,"","",1)
         rollback work
         return false
      end if

      delete from vmn_file
         where vmn01 = p_ecb01 and vmn02 = p_ecb02
         and vmn03 = p_ecb03

      if sqlca.sqlcode then
         call cl_err3("del","vmn_file",p_ecb01,p_ecb02,sqlca.sqlcode,"","",1)
         rollback work
         return false
      end if

      delete from vnm_file
         where vnm00 = p_ecb01 and vnm01 = p_ecb02
         and vnm02 = p_ecb03 and vnm03=b_ecb.ecb06

      if sqlca.sqlcode then
         call cl_err3("del","vnm_file",p_ecb01,p_ecb02,sqlca.sqlcode,"","",1)
         rollback work
         return false
      end if
      ...
      delete from  sgc_file
      where sgc01=p_ecb01 and sgc02=p_ecb01
         and sgc03=p_ecb03
         and sgc012 = g_ecu.ecu012     #fun-a50081
      if sqlca.sqlcode then
         call cl_err3("del","sgc_file",p_ecb01,p_ecb02,sqlca.sqlcode,"","",1) #fun-660091
         let g_success = 'N'
      end if

      delete from  eco_file
      where eco01=p_ecb01 and eco02=p_ecb02
         and eco03=p_ecb03
         and eco012 = g_ecu.ecu012
      if sqlca.sqlcode then
         call cl_err3("del","eco_file",p_ecb01,p_ecb02,sqlca.sqlcode,"","",1) #fun-660091
         let g_success = 'N'
      end if
