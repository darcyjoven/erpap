...
DEFINE   g_zat                 DYNAMIC ARRAY OF RECORD #定義Field
           ...
           zat08               LIKE tc_zat_file.zat08,                             
           zat09               LIKE tc_zat_file.zat09,                             
   ...
   WHILE TRUE
      ...
      CASE g_page_choice
         ...
         WHEN "query_sql"
              ...
              if g_action_choice = 'sql_command' then
      ...
      CASE g_action_choice
         ...
         when 'del_zal'
            IF cl_chk_act_auth() THEN
               delete from tc_zal_file where zal01 = g_zai.zai01
               delete from tc_zat_file where zat01 = g_zai.zai01
               call p_query_sql_b_fill(" 1=1")
            end if
...
FUNCTION p_query_field_b()                                 # 單身
  ...
  #LET l_str = p_query_cut_spaces(g_zak.zak03)
   ...
   INPUT ARRAY g_zat WITHOUT DEFAULTS FROM s_zat.*
      ...
      AFTER FIELD zat08
         case g_zat[l_w_ac].zat08
            when '1'
               let g_zat[l_w_ac].zat09 = 6
            when '2'
               let g_zat[l_w_ac].zat09 = 2
            when '3'
               let g_zat[l_w_ac].zat09 = 3
            when '4'
               let g_zat[l_w_ac].zat09 = 10
            when '5'
               let g_zat[l_w_ac].zat09 = 0
            when '6'
               let g_zat[l_w_ac].zat09 = "yyyy/mm/dd"
         end case
      BEFORE FIELD zat06
...
FUNCTION p_query_parse_sql(p_cmd)
    ...
    define l_temp2 dynamic array of string
    define l_temp string
    define l_temp_idx integer
 
--  抓取所有的語言別
    DECLARE lang_arr CURSOR FOR
     SELECT UNIQUE gay01 FROM gay_file where gay01 ='2' ORDER BY gay01 #darcy:2023/05/30 #只要简体中文即可
    LET l_i=1
   ...
   #LET l_str=p_query_cut_spaces(g_zak.zak02)
    ...
    WHILE l_tok.hasMoreTokens()
          ...
          #darcy:2023/05/30 add s---
          # 如果只有有'('  将等待下一个')''
          if l_feld[l_i].getIndexOf('(',1)>0 and l_feld[l_i].getIndexOf(')',1) =0 then
            while true
               let l_feld[l_i] =l_feld[l_i] ,',', l_tok.nextToken()
               let l_feld[l_i]=l_feld[l_i].trim()
               if l_feld[l_i].getIndexOf(')',1) > 0 then
                  exit while
               end if
            end while
          end if
          #darcy:2023/05/30 add e---
          LET l_i=l_i+1
    END WHILE
    ...
    IF p_cmd = 'a' THEN
       ...
       FOR l_i = 1 TO l_feld_cnt
           ...
           END IF
           #darcy:2023/05/30 add s---
           #   如果中间有空格，只通过最后一个空格后的值取说明
           let l_temp = l_feld_tmp
           if l_temp.getIndexOf(" ",1) > 0 then
               LET l_tok = base.StringTokenizer.create(l_feld_tmp," ")
               let l_temp_idx=1
               call l_temp2.clear()
               while l_tok.hasmoretokens()
                  let l_temp2[l_temp_idx] = l_tok.nexttoken()
                  let l_temp_idx = l_temp_idx + 1
               end while
               for l_temp_idx = l_temp2.getlength() to 1 step - 1
                  if not cl_null(l_temp2[l_temp_idx]) then
                     let l_feld_tmp = l_temp2[l_temp_idx]
                     exit for
                  end if
               end for
            end if
