...
CREATE TABLE p324_tmp(
   ...
   LET l_sql = " SELECT cdl01,cdl02,cdl03,cdl04,cdl05,cdl06,cdl07,cdl10,tlf907,tlf905,tlf906,tlf10,tlf902,0 dj,'10' lb",
               "   FROM cdl_file ",
               "   LEFT JOIN (SELECT tlf907,tlf905,tlf906,tlf10,tlf902,tlf19,tlf14,tlf13,tlf06,tlf07,tlfccost,tlf01 ",
               "                FROM tlf_file ",
   ...
   #2、筛选INSTR(tlf01, '.')>1  and tlf021(仓库）in (XBC,P002) 的料件，902单位成本=WIP杂收金额zsje/WIP杂收数量ta_ccc217 就是ta_ccp_file中的ta_cccud07
   ...
   UPDATE p324_tmp SET lb = '3' WHERE INSTR(cdl07, '.')>1 AND tlf902 IN ('XBC','P002')
   ...
   LET l_sql = "  MERGE INTO p324_tmp o ",
               ...
               "          SELECT ta_ccc01,'3',ta_cccud07 FROM ta_ccp_file WHERE ta_ccc02 = ",tm.yy," AND ta_ccc03 = ",tm.mm," AND ta_cccud07 <> 0 AND INSTR(ta_ccc01, '.') > 1 UNION ALL ",
               "          SELECT tlf01,(CASE lx WHEN '杂收' THEN '4' ELSE '5' END) lx,SUM(zsje/zsl) FROM cxcq910_file WHERE yy = ",tm.yy," AND mm = ",tm.mm," AND INSTR(tlf01, '-')>1 AND zsl <> 0 GROUP BY tlf01,lx UNION ALL ",
               ...
               "  WHEN MATCHED THEN ",
               " UPDATE SET o.cdl10 = n.dj * o.tlf10 "
   ...
   LET l_sql = "  MERGE INTO cdl_file o ",
               "  USING (SELECT cdl01,cdl02,cdl03,cdl04,cdl05,cdl06,cdl07,SUM(cdl10*tlf907) cdl10 FROM p324_tmp GROUP BY cdl01,cdl02,cdl03,cdl04,cdl05,cdl06,cdl07 ) n ",
