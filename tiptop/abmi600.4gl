...
FUNCTION i600_bma01(p_cmd)  #主件料件
    ...
    LET g_errno = ' '
    SELECT  ima02,ima021,ima55, ima08,imaacti,ima05,imaud07,imaud10
       INTO l_ima02,l_ima021,l_ima55, g_ima08_h,l_imaacti,
            l_ima05,l_imaud07,l_imaud10
       FROM ima_file
      WHERE ima01 = g_bma.bma01
...
FUNCTION i600_e_work_dhy()
   ...
   DEFINE i      SMALLINT
   LET i =0
   LET l_sql = "SELECT bma01 FROM bma_file"               
   PREPARE i600_e_pb1 FROM l_sql
   DECLARE bmb_e_curs1 CURSOR FOR i600_e_pb1
   BEGIN WORK 
   FOREACH bmb_e_curs1 INTO g_bma.bma01
      #MESSAGE g_bma.bma01
      CALL i600_e_work()
      #MESSAGE g_bma.bma01
      LET i = i + 1   
