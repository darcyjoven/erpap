...
DEFINE l_cmd        STRING   #FUN-730047
    ...
    LET g_syspass = "change_on_install"     #FUN-730047  
   
    IF p_db_type="ORA" THEN    #FUN-730047
...
#display "l_command[l_k].cmd:",l_command[l_k].cmd
    ...
    CALL l_command.deleteElement(l_k)
    display "l_command.getLength:",l_command.getLength()
    ...
    IF p_backup='Y' THEN  --需要備份的情況
       ...
       IF SQLCA.SQLCODE THEN
          ...
          ELSE
             LET l_cmd = "echo ""conn sys/" , g_syspass , "@$ORACLE_SID as sysdba\ndrop user qazzaq cascade;""|sqlplus /nolog > /dev/null 2>&1"     #FUN-730047
       ...
       FOR l_k=1 TO l_command.getLength()    
           IF l_command[l_k].cmd.subString(1,12)="create table" THEN
              FOR l_i=14 TO l_command[l_k].cmd.getLength()
                  ...
                  END IF
              END FOR
              LET readsql_channel=base.channel.create()
              CALL readsql_channel.setdelimiter("")
              LET l_str=l_command[l_k].cmd.subString(14,l_start-2),".sch.src"
              CALL readsql_channel.openfile(l_str CLIPPED, "r" )
              WHILE readsql_channel.read(l_str)
                    IF l_str.subString(1,12)="create table" THEN
                       LET l_flag=1
#                       LET l_str1[l_j]=''
                    END IF
                    IF l_flag=1 THEN
#                       IF l_str.getLength()>30 THEN
#                          LET l_str1[l_j] = l_str1[l_j]," ",l_str.subString(1,30)
#                       ELSE
                       LET l_str1[l_j] = l_str1[l_j]," ",l_str.subString(1,l_str.getLength())
#                       END IF
                    END IF
                    IF l_str.subString(1,2)=");" THEN
                       LET l_flag=0
                       LET l_str1[l_j]=l_str1[l_j].subString(1,l_str1[l_j].getLength()-1)
                       LET l_j=l_j+1
                    END IF
                    IF l_str.subString(1,12)="create uniqu" OR
                       l_str.subString(1,12)="create      " THEN
                       LET l_str1[l_j]=l_str.subString(1,l_str.getLength()-1)
                       LET l_j=l_j+1
                    END IF
              END WHILE
              CALL readsql_channel.close()
              FOR l_i=l_l TO l_j-1
                  EXECUTE IMMEDIATE l_str1[l_i]
                  LET l_l=l_l+1
#display "l_str1[l_i]:",l_str1[l_i]
                  IF SQLCA.SQLCODE THEN
