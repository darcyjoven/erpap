...
MAIN
   ...
   DISPLAY "g_rpt_name:",g_rpt_name
    ...
    LET g_sql ="ecu01.ecu_file.ecu01,",
               ...
               ,",ecbud10.type_file.chr30,"
               ,",ecbud11.type_file.chr30"
               #darcy:2024/04/18 add s---
...
FUNCTION cecr001()
  ...
  #公司全名zo02、公司地址zo041、公司電話zo05、公司傳真zo09
   ...
   LET g_sql = "INSERT INTO ",g_cr_db_str CLIPPED,l_table CLIPPED,
               " values(?,?,?,?,?, ?,?,?,?,?, ?,?,?,?,?  ,?,?,?,?,? ,?,?)"   #add ? by guanyao160928 #darcy:2024/04/18 add 2?  
   ...
   LET l_sql = "SELECT ecu01,'','',trim(imaud07),trim(imaud10),ecb03,ecb06,ecb17,ecb40,trim(ecbud04),trim(ecbud05),trim(ecbud03),trim(concat(concat(ecbud02,'    '),ecbud17)) as ecbud02,ima02,'','',tc_sma06 as imaud06,ecu02,imaud09,concat(ecbud02,ecbud17) as ecbud17",
               " ,ecuud10,ecuud11",#darcy:2024/04/18 add 
               "  FROM ecb_file,ecu_file LEFT JOIN (select ima_file.*,tc_sma06 from tc_sma_file,ima_file where tc_sma01='csmi102' and tc_sma02='aimi100' and tc_sma03 = imaud06) ON ima01=ecu01 ",
   ...
   FOREACH decl_aaa INTO sr.*
     ...
     END IF 
     #darcy:2024/04/18 add s---
     let sr.ecbud10 = iif(sr.ecbud10=='1',"Y","N")
     if sr.ecbud11=='1' then
         let sr.ecbud11 = "UL1"
     end if
     if sr.ecbud11=='2' then
         let sr.ecbud11 = "UL2"
     end if 
     #darcy:2024/04/18 add e---
