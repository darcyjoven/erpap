...
{         WHEN "delete"
         ...
         when "fastexcel"
            if cl_chk_act_auth() then
               call list.clear()
               let list[1].name = "s_tc_imb"
               let list[1].node = base.typeinfo.create(g_tc_imb)
               let list[2].name = "s_tc_imb_1"
               let list[2].node = base.typeinfo.create(g_tc_imb_1)
               let list[3].name = "s_tc_imb_2"
               let list[3].node = base.typeinfo.create(g_tc_imb_2)
               let list[4].name = "s_tc_imb_3"
               let list[4].node = base.typeinfo.create(g_tc_imb_3)
...
DEFINE l_sql1 STRING
   ...
   create index cect001_tmp_idx on cect001_tmp(sgm01,sgm03)
