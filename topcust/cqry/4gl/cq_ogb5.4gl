...
FUNCTION ogb5_cqry_prep_result_set()
   ...
   define l_ogb12 like ogb_file.ogb12  #darcy:2023/06/13 add
...
#modify by jiangln 170524 end------- 
   ...
   FOREACH lcurs_qry INTO lr_qry.*
      ...
      IF l_omf917 >= lr_qry.ogb917 AND mi_multi_sel THEN
         CONTINUE FOREACH
      END IF
      #darcy:2023/06/13 add s---
      # 出货单全部项次是否已经全部开票
      LET l_omf917 = 0 
      let l_ogb12 = 0 
      SELECT SUM(ogb917) into l_ogb12  FROM ogb_file
       WHERE ogb01 = lr_qry.oga01_1
      SELECT SUM(omf917) into l_omf917
        FROM omf_file,oga_file
      WHERE oga011 =lr_qry.oga01_1 AND omf11=oga01 AND omf08!='X'
      if l_omf917>=l_ogb12 then
         continue foreach
      end if
      #darcy:2023/06/13 add e---
      #MOD-CA0092--add--end
