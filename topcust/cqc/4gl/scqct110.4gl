...
#FUN-C30117---add---END
   ...
   # END CASE
   if g_prog = 'cqct110' then
      let g_sql = g_sql clipped," and qcs00 ='3' "
   end if
...
FUNCTION t110_set_entry(p_cmd)
   DEFINE p_cmd   LIKE type_file.chr1    #No.FUN-680104 VARCHAR(1)
     ...
     CALL cl_set_comp_entry("qcs21,qcsud01,qcsud03,qcsud04,qcsud05,qcsud06,qcsud14",TRUE)
     CALL cl_set_comp_entry("qcsud08,qcsud11,qcsud12",TRUE) #darcy:2022/10/17 add
...
#FUN-BC0104 --------------End----------------
   ...
   #      FROM qck_file WHERE qck01=g_ima109 AND qck02=g_qct[l_ac].qct04
      ...
      AFTER FIELD qct11
         ...
         # IF g_qct[l_ac].qct11>0 AND l_qcd07='Y'AND g_qcz.qcz01='Y' THEN #mark darcy:2022/10/31 
         #darcy:2022/11/14 add s---
         if g_qct[l_ac].qct04 not matches '*包装外观*' and
            g_qct[l_ac].qct04 not matches '*材料外观*' and
            g_qct[l_ac].qct04 not matches '*尺寸*' and
            g_qct[l_ac].qct04 not matches '*漂锡*'  then
         #darcy:2022/11/14 add e---
         IF g_qct[l_ac].qct11>0 THEN #add darcy:2022/10/31 
