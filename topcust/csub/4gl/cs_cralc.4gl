        ...
        END CASE
         ...
         #FUN-910088 --add--end--
           UPDATE sfa_file SET sfa05 = l_total2,
            ...
            # 处理自动替代料
            if g_sfa.sfa26 matches "[12]" then
               if cs_cralc_sub(g_sfa.*) then
               end if
            end if
           #darcy:2023/05/04 add s---
           #FUN-C30274 --START--
...
define l_cbmi255    like tc_bmj_file.tc_bmj07    #darcy:2023/04/13 add
       
   IF cl_null(p_bmb08) THEN LET p_bmb08=0 END IF    #FUN-A60088
   #darcy:2023/04/13 add s---
   # if g_user = 'tiptop' or g_user = '31186' then
      # 计算cbmi255 维护的良率
      # 最终损耗率 = abmi600 损耗率 + （100 - cbmi255 良率）
...
# 自动取替代产生
function cs_cralc_sub(p_sfa)
   define p_sfa  record like sfa_file.*
   define l_sfa  record like sfa_file.*
   define l_sql  string
   define l_new_qty  like sfa_file.sfa05
   define l_bmd  record
   ...
   # if g_user !='tiptop' and g_user !='31186' then
   #    return true
   # end if
   ...
   declare sub_bmd_curs cursor for
   select bmd04, bmd07, bmd03, ima63, ima63_fac, ima86, ima86_fac
     from bmd_file
     left outer join ima_file on bmd04 = ima01
    where bmd01 = p_sfa.sfa03
      and bmd02 <> '3'
      and (bmd08 = g_sfb.sfb05 or bmd08 = 'ALL')
      and (bmd05 is null or bmd05 <= g_sfb.sfb071)
      and (bmd06 is null or bmd06 > g_sfb.sfb071)
      and bmdacti = 'Y' and ta_bmd01 = 'Y'

   open sub_bmd_curs
   fetch sub_bmd_curs into l_bmd.*
   free sub_bmd_curs
   ...
   let l_new_qty = p_sfa.sfa05 - p_sfa.sfa06
   let l_new_qty = l_new_qty * l_bmd.bmd07
   let l_new_qty = s_digqty(l_new_qty,l_bmd.ima63)

   let l_sfa.* = p_sfa.*
   let l_sfa.sfa03 = l_bmd.bmd04
   let l_sfa.sfa04 = l_new_qty
   let l_sfa.sfa05 = l_new_qty
   let l_sfa.sfa06 = 0
   let l_sfa.sfa061 = 0
   let l_sfa.sfa062 = 0
   let l_sfa.sfa063 = 0
   ...
   let l_sfa.sfa065 = 0
   let l_sfa.sfa066 = 0
   if l_sfa.sfa08 is null then
      let l_sfa.sfa08 = ' '
   end if
   let l_sfa.sfa16 = 0
   let l_sfa.sfa161 = 0
   let l_sfa.sfa12 = l_bmd.ima63
   let l_sfa.sfa13 = l_bmd.ima63_fac
   let l_sfa.sfa14 = l_bmd.ima86
   let l_sfa.sfa15 = l_bmd.ima86_fac
   let l_sfa.sfa25 = l_new_qty
   let l_sfa.sfa26 = iif(l_sfa.sfa26='1','U','S')
   let l_sfa.sfa28 = l_bmd.bmd07
   --let l_sfa.sfa100 = iif(cl_null(l_sfa.sfa100),0,l_sfa.sfa100)
   insert into sfa_file values (l_sfa.*)
   update sfa_file set sfa05 = 0,
                       sfa26 = (case sfa26 when '1' then '3' else '4' end),
                       sfa25 = 0 - p_sfa.sfa05
    where sfa01 = p_sfa.sfa01
      and sfa03 = p_sfa.sfa03
      and sfa08 = p_sfa.sfa08
      and sfa12 = p_sfa.sfa12
      and sfa27 = p_sfa.sfa27
