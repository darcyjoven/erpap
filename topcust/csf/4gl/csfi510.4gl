...
MAIN
   ...
   #      FIELD ORDER FORM                   #整個畫面欄位輸入會依照p_per所設定的順序(忽略4gl寫的順序)  #FUN-730075  #FUN-A60028
    ...
    LET g_argv2 = '1'
    LET g_tc_sfd06 = '1'
    call cl_set_act_visible("detail_vip",false) #darcy:2024/04/26 add 隐藏detail_vip按钮
...
FUNCTION i510_menu()
   ...
   DEFINE l_ok         LIKE tc_sfd_file.tc_sfd01
   DEFINE kt_no        LIKE tc_sfd_file.tc_sfd01
   DEFINE kt_no_2        LIKE tc_sfd_file.tc_sfd01
   DEFINE kt_reason    LIKE tc_sfe_file.tc_sfeud01
  #No.18010101--begin--
   DEFINE l_ret        RECORD
       ...
       #@WHEN "確認"
         WHEN "confirm"
            IF cl_chk_act_auth() THEN
               ...
               SELECT tc_sfe02 INTO kt_no_2 FROM  tc_sfe_file WHERE tc_sfe01 = g_tc_sfd.tc_sfd01 #xyb-MES调用接口
               IF (kt_no_2[1,3] = 'MKT' OR kt_no_2[1,3] = 'MSG') THEN 
                   CALL getMesktReasonR(kt_no_2) RETURNING l_ok,kt_no,kt_reason
                     IF (l_ok = 'Y') THEN
                        UPDATE tc_sfd_file SET tc_sfdud01 = kt_reason WHERE tc_sfd01 = g_tc_sfd.tc_sfd01
                     ELSE 
                        CALL cl_err(kt_no_2,'xyb-510',1) 
                    END IF 
                    END IF  #xyb-MES调用接口
...
#@    ON ACTION 套數
    ...
    #@ON ACTION 取消確認
      ...
      #darcy:2024/04/26 add s---
      on action detail_vip
         let g_action_choice = "detail_vip"
         exit dialog
      #darcy:2024/04/26 add e---
