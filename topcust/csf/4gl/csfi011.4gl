...
DEFINE 
     g_gfe           DYNAMIC ARRAY OF RECORD    #程式變數(Program Variables)
        ...
        ,tc_baa04      LIKE tc_baa_file.tc_baa04         #darcy:2023/02/21 add
                    END RECORD,
...
define g_timestamp           varchar(40)  #darcy:2024/02/01
...
FUNCTION i101_menu()
   ...
   define l_cmd   string #darcy:2024/02/01 add
   ...
   WHILE TRUE
      ...
      CASE g_action_choice
         ...
         WHEN "exporttoexcel"   #No.FUN-4B0020
            ...
            END IF
         #darcy:2024/02/01 add s---
         when "query_csfq027"
            if cl_chk_act_auth() then
               let l_cmd = " tc_bac01='",g_gfe[l_ac].tc_baa01 clipped,"'"
               let l_cmd = 'p_query "csfq027" "',l_cmd,'"'
               call cl_cmdrun_wait(l_cmd)
            end if
         #darcy:2024/02/01 add e---
...
DEFINE l_azo06      LIKE azo_file.azo06                  #CHI-BB0048 add
    ...
    LET g_forupd_sql = "SELECT  tc_baa01,tc_baa02,tc_baa03,tc_baa04 FROM tc_baa_file",   #FUN-A30030 ADD POS #darcy:2023/02/21 add tc_baa04
...
#No.FUN-570110 --end     
            ...
            LET g_gfe[l_ac].tc_baa04 = 0       #darcy:2023/02/21 add
        ...
        AFTER INSERT
           ...
           INSERT INTO tc_baa_file(tc_baa01,tc_baa02,tc_baa03,tc_baa04)  #FUN-A30030 ADD POS #darcy:2023/02/21 add 
                         VALUES(g_gfe[l_ac].tc_baa01,g_gfe[l_ac].tc_baa02,
                                g_gfe[l_ac].tc_baa03,g_gfe[l_ac].tc_baa04)  #FUN-A30030 POS      #No.FUN-980030 10/01/04  insert columns oriu, orig #darcy:2023/02/21
           IF SQLCA.sqlcode THEN
#             CALL cl_err(g_gfe[l_ac].tc_baa01,SQLCA.sqlcode,0)   #No.FUN-660131
           ...
           ELSE
              ...
              #           (3)功能選項：insert(新增),update(修改),delete(刪除)
               #darcy:2024/02/01 add s---
               let g_timestamp = current year to fraction(3)
               insert into tc_bac_file values(
                  g_gfe[l_ac].tc_baa01,g_gfe[l_ac].tc_baa02,
                  g_gfe[l_ac].tc_baa03,g_gfe[l_ac].tc_baa04,
                  g_user,g_timestamp,'insert'
               )
               #darcy:2024/02/01 add e---
        ...
        ON ROW CHANGE
           ...
           ELSE
               ...
               UPDATE tc_baa_file SET tc_baa01=g_gfe[l_ac].tc_baa01,
                                   ...
                                   ,tc_baa04=g_gfe[l_ac].tc_baa04 #darcy:2023/02/21 add
...
#                 CALL cl_err(g_gfe[l_ac].tc_baa01,SQLCA.sqlcode,0)   #No.FUN-660131
       ...
       #    EXIT INPUT
        ...
        #darcy:2024/02/18 add s---
        on action query_csfq027
            let g_action_choice = 'query_csfq027'
            exit input
        #darcy:2024/02/18 add e---
...
FUNCTION i101_b_askkey()
    CLEAR FORM
   CALL g_gfe.clear()
    CONSTRUCT g_wc ON tc_baa01,tc_baa02,tc_baa03,tc_baa04            #darcy:2023/02/21
            FROM s_gfe[1].tc_baa01,s_gfe[1].tc_baa02,s_gfe[1].tc_baa03,s_gfe[1].tc_baa04    #FUN-A30030 ADD POS #darcy:2023/02/21
...
DEFINE
    ...
    LET g_sql =
        "SELECT tc_baa01,tc_baa02,tc_baa03,tc_baa04 ",                   #FUN-A30030 ADD POS
        " FROM tc_baa_file",
...
#   LET l_name = 'csfi011.out'
    ...
    IF g_zz05='Y' THEN                                      #No.FUN-760083
       CALL cl_wcchp(g_wc,'tc_baa01,tc_baa02,tc_baa03,tc_baa04') #FUN-A30020 ADD POS     #No.FUN-760083
...
# select  tc_shb04 , tc_shb14, tc_shb05,ima02,ima021,imaud07,imaud10,tc_shb06 ,tc_shb08 ,ecd02 ,tc_shb09 ,eca02  ,tc_shb12 ,tc_shb12/imaud10,tc_shb12/imaud10*b*c/1000000,
# tc_baa02,tc_baa03,19.32*tc_baa02*tc_baa03*tc_shb12/imaud10/0.683/100
# from tc_shb_file left join ecd_file on ecd01=tc_shb08 
#   left join eca_file on eca01=tc_shb09
#   left join ima_file on ima01=tc_shb05
#   left join mianji on a=substr(tc_shb05,1,6)
#   left join tc_baa_file on tc_baa01=substr(tc_shb05,1,6)
# where  tc_shb01='2' 
