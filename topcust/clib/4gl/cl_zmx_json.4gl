...
DEFINE l_returnmsg      String
    ...
    END IF 
        ...
        #darcy:2024/06/24 add M.MJ 作为消耗性料件
        IF cl_null(l_cnt) THEN LET l_cnt = 0 END IF 
        IF l_cnt > 0 THEN 
 ...
 #      LET l_detail.amount         =    l_pmn.pmn88t                                             #金额
    ...
    LET g_jsonstr = l_json_str
    LET g_returnstr = l_returnmsg
    IF l_returnstatus = '0' AND l_returnmsg NOT MATCHES  '*失败*' AND l_returnmsg NOT MATCHES '*料件*' AND l_returnmsg NOT MATCHES '*物料*'  THEN 
    	LET l_ret.success = 'Y' 
    	ELSE 
    	LET l_ret.success = 'N' 
    END IF 
